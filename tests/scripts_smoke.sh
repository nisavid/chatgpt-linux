...
JSON
    printf '%s\n' '# Conversation Mode' > "$integrations_root/conversation-mode/README.md"
    cat > "$integrations_root/example-integration/integration.json" <<'JSON'
{"id":"example-integration","title":"Example Linux Feature","description":"Developer sample."}
JSON
    printf '%s\n' '# Example Linux Feature' > "$integrations_root/example-integration/README.md"
...
JSON
    printf '%s\n' '# Read Aloud' > "$integrations_root/read-aloud/README.md"
...
JSON
    printf '%s\n' '# Read Aloud MCP' > "$integrations_root/read-aloud-mcp/README.md"
...
JSON
    printf '%s\n' '# Remote Mobile Control' > "$integrations_root/remote-mobile-control/README.md"
}
...
JSON
    ...
    printf '%s\n' '# Private package resource' > "$integration_dir/README.md"
    printf '%s\n' 'package secret' > "$integration_dir/secret.txt"
    cat > "$integration_dir/integration.json" <<'JSON'
{
  "id": "private-package",
  "packageResources": [
    {
...
test_port_integration_package_hook_discovery_failure_blocks_build() {
    info "Checking port integration package hook discovery failure blocks package staging"
    ...
    mkdir -p "$root" "$integrations_root/bad-package-hook"
    make_fake_app "$app_dir"
    printf '%s\n' '{"enabled":[]}' > "$integrations_root/integrations.example.json"
    cat > "$integrations_root/bad-package-hook/integration.json" <<'JSON'
{
  "id": "bad-package-hook",
...
JSON
    ...
    if (
        ...
        # shellcheck disable=SC1091
        source "$REPO_DIR/scripts/lib/package-common.sh"
        run_port_integration_package_hooks "$root" "deb"
    ) >"$output_log" 2>&1; then
        fail "Expected package hook discovery failure to stop package staging"
    fi

    assert_contains "$output_log" "Failed to discover port integration package hooks for deb"
    assert_contains "$output_log" "packageHook 1 not found"
}

test_port_integration_package_dependency_failure_propagates() {
    info "Checking port integration dependency discovery failure blocks metadata rendering"
    ...
    make_fake_app "$app_dir"
    mkdir -p "$integrations_root/bad-package-dependency"
    printf '%s\n' '{"enabled":[]}' > "$integrations_root/integrations.example.json"
    printf '%s\n' '# Bad Package Dependency' \
        > "$integrations_root/bad-package-dependency/README.md"
    cat > "$integrations_root/bad-package-dependency/integration.json" <<'JSON'
{
...
JSON
    ...
    (
        ...
        # shellcheck disable=SC1091
        source "$REPO_DIR/scripts/lib/package-common.sh"
        port_integration_package_dependency_suffix deb "$app_dir"
    ) >"$output_log" 2>&1
    local rc=$?
    set -e

    [ "$rc" -ne 0 ] \
        || fail "Invalid port integration dependencies were silently omitted"
...
test_pacman_builder_without_updater_transition_hook() {
    ...
    cat > "$integrations_root/polkit-runtime/integration.json" <<'JSON'
{
  "id": "polkit-runtime",
...
exit 0
SCRIPT
    chmod +x "$bin_dir/makepkg" "$bin_dir/cargo" "$bin_dir/pacman"
    ...
    assert_not_contains "$capture_dir/PKGBUILD" "'curl'"
...
SCRIPT
    ...
    if APP_DIR_OVERRIDE="$rpm_no_webview_app_dir" PACKAGE_WITH_UPDATER=0 PACKAGE_VERSION="2026.03.24.120000+missing" bash "$REPO_DIR/scripts/build-rpm.sh" >"$rpm_no_webview_log" 2>&1; then
    ...
    if APP_DIR_OVERRIDE="$rpm_app_dir" PACKAGED_RUNTIME_SOURCE="$workspace/does-not-exist.sh" PACKAGE_WITH_UPDATER=0 PACKAGE_VERSION="2026.03.24.120000+missing" bash "$REPO_DIR/scripts/build-rpm.sh" >"$rpm_log" 2>&1; then
...
SCRIPT
    ...
    [ "$(cat "$aria2_success/ChatGPT.dmg")" = "aria2-download" ] || fail "Expected aria2c to download the DMG"
    ...
    assert_contains "$aria2_success/aria2.log" "--out=ChatGPT.dmg.part"
    ...
    [ "$(cat "$aria2_fallback/ChatGPT.dmg")" = "curl-fallback" ] || fail "Expected curl fallback after aria2c failure"
    ...
    assert_file_not_exists "$aria2_fallback/ChatGPT.dmg.part"
    assert_file_not_exists "$aria2_fallback/ChatGPT.dmg.part.aria2"
...
JSON
    ...
    node - "$config" <<'NODE' || fail "Expected setup-native wizard to preserve port integration settings"
const fs = require("node:fs");
...
test_setup_native_wizard_rejects_integrations_without_readme() {
    info "Checking setup-native wizard rejects undocumented port integrations"
    ...
    assert_contains "$output_log" "must include README.md next to integration.json"
    assert_json_enabled_equals "$config" '[]'
}
...
test_setup_native_wizard_accepts_numbered_integration_selection() {
    info "Checking setup-native wizard accepts numbered integration selections"
    local workspace="$TMP_DIR/setup-native-numbered-integrations"
    local integrations_root="$workspace/port-integrations"
    local config="$workspace/integrations.json"
    local output_log="$workspace/output.log"
    local fake_home="$workspace/home"
    ...
    if ! command -v script >/dev/null 2>&1; then
        info "Skipping numbered integration selection smoke test because script(1) is unavailable"
        return
    fi
    ...
    assert_contains "$output_log" "Enable integration ids or numbers for the next build"
    assert_contains "$output_log" "Disable integration ids or numbers for the next build"
}

test_setup_native_wizard_rejects_out_of_range_integration_numbers() {
    info "Checking setup-native wizard explains out-of-range feature numbers"
    local workspace="$TMP_DIR/setup-native-integration-number-range"
    local integrations_root="$workspace/port-integrations"
    local config="$workspace/integrations.json"
    local output_log="$workspace/output.log"
    ...
    assert_contains "$output_log" "Integration number 99 is out of range for enable"
    assert_contains "$output_log" "Use integration ids, numbers, or ranges like 1,3-4."
    assert_json_enabled_equals "$config" '[]'
}
...
test_setup_native_wizard_lists_local_integrations() {
    info "Checking setup-native wizard discovers user-local port integrations"
    local workspace="$TMP_DIR/setup-native-local-integration"
    local integrations_root="$workspace/port-integrations"
    local config="$workspace/integrations.json"
    local output_log="$workspace/output.log"

    make_wizard_integration_root "$integrations_root"
    mkdir -p "$integrations_root/local/local-tool"
    printf '%s\n' '{"id":"local-tool","title":"Local Tool","description":"User-local integration."}' \
        > "$integrations_root/local/local-tool/integration.json"
    ...
    assert_contains "$output_log" "Enabled port integrations: local-tool"
}
...
test_setup_native_wizard_blank_interactive_cleanup_ids_skip_cleanup() {
    info "Checking setup-native wizard skips cleanup when interactive integration ids are blank"
...
test_update_nix_hashes_verifies_changed_pins() {
    ...
    assert_contains "$fixture/output.log" "Nix builds succeeded after refreshing the official DMG pins and ChatGPT.dmg hash."
...
test_update_nix_hashes_supports_focused_verification_output() {
    ...
    NIX_VERIFY_OUTPUTS=".#checks.x86_64-linux.nix-port-integrations-multi-feature" \
        run_update_nix_hash_fixture "$(basename "$fixture")" 0 "$hash_b"
...
test_native_module_rebuild_accepts_prebuilt_source() {
    ...
    case "$(uname -m)" in
        ...
        *) fail "Unsupported native module smoke-test architecture: $(uname -m)" ;;
    esac
...
test_packaged_runtime_keeps_managed_node_out_of_user_service_path() {
    info "Checking packaged runtime keeps app PATH out of user service activation"
    ...
    local import_args="DISPLAY WAYLAND_DISPLAY DBUS_SESSION_BUS_ADDRESS XAUTHORITY XDG_RUNTIME_DIR HYPRLAND_INSTANCE_SIGNATURE YDOTOOL_SOCKET"
    local -a captures
...
EOF
    ...
    (
        ...
        [ "${captures[0]:-}" = "systemctl|$managed_node_bin:$user_path|$homebrew_prefix|--user import-environment $import_args" ] \
            || fail "Expected systemd activation to exclude PATH and HOMEBREW_PREFIX"
        [ "${captures[1]:-}" = "dbus|$managed_node_bin:$user_path|$homebrew_prefix|--systemd $import_args" ] \
            || fail "Expected D-Bus activation to exclude PATH and HOMEBREW_PREFIX"
        [ "${#captures[@]}" -eq 2 ] \
        ...
        [ "${captures[0]:-}" = "systemctl|$fallback_path||--user import-environment $import_args" ] \
            || fail "Expected systemd to keep the fallback PATH out of activation imports"
        [ "${captures[1]:-}" = "dbus|$fallback_path||--systemd $import_args" ] \
            || fail "Expected D-Bus to keep the fallback PATH out of activation imports"
        [ "${#captures[@]}" -eq 2 ] \
            || fail "Expected fallback display/session imports only"
    )
}
...
if 'run_cold_start_hooks' not in runtime_body:
    raise SystemExit("cold start must run port-integration-staged hooks before Electron launches")
...
test_upstream_bundled_skills_staging() {
    ...
    (
        ...
        source "$REPO_DIR/scripts/lib/bundled-plugins.sh"
        stage_linux_computer_use_plugin() { return 1; }
        stage_browser_plugin_from_official_app() { return 1; }
        stage_chrome_plugin_from_official_app() { return 1; }
        install_browser_use_node_repl_resource() { return 0; }
        install_bundled_plugin_resources "$app_dir"
    ) >"$output_log" 2>&1
...
test_browser_use_node_repl_glibc_pidfd_patch_static() {
    ...
    assert_contains "$REPO_DIR/scripts/lib/bundled-plugins.sh" "is_browser_use_node_repl_elf_compatible"
    assert_contains "$REPO_DIR/scripts/lib/bundled-plugins.sh" "browser_use_node_repl_required_glibc_versions"
    ...
    assert_contains "$REPO_DIR/scripts/lib/bundled-plugins.sh" 'command -v readelf'
}
...
test_user_local_prepare_build_repo_copies_enabled_local_features() {
    info "Checking user-local managed checkout stages enabled local integrations"
    ...
    printf '%s\n' '# Linux Features' > "$source_repo/port-integrations/README.md"
    ...
    printf '%s\n' '# Repo Feature' > "$source_repo/port-integrations/repo-feature/README.md"
...
EOF
    ...
    assert_file_exists "$staged_local_feature/integration.json"
...
test_webview_integrity_manifest_ignores_import_text_in_js_strings_and_comments() {
    ...
    printf "%s\n" "<title>Codex</title><script type=\"module\" src=\"./assets/app-test.js\"></script>" > "$install_dir/content/webview/index.html"
    cat > "$install_dir/content/webview/assets/app-test.js" <<'EOF'
console.log("example: import './missing-string.js'");
// import "./missing-line-comment.js";
/*
export { thing } from "./missing-block-comment.js";
import("./missing-dynamic-comment.js");
*/
import "./present.js";
EOF
...
EOF
    ...
    if git -C "$source_repo" merge --no-ff feature >/dev/null 2>&1; then
        fail "Expected merge to conflict in unmerged overlay smoke test"
    fi
    git -C "$source_repo" ls-files -u -- conflict.txt | grep -q . \
        || fail "Expected conflict.txt to remain unmerged in source checkout"
...
test_pacman_updater_upgrade_only_starts_enabled_service() {
    info "Checking pacman upgrade preserves updater service enablement choice"
    if [ "$(id -u)" -eq 0 ]; then
        info "Skipping pacman updater lifecycle smoke test while running as root"
        return 0
    fi

    local workspace="$TMP_DIR/pacman-updater-lifecycle"
    local bin_dir="$workspace/bin"
    local app_dir="$workspace/app"
    local dist_dir="$workspace/dist"
    local capture_dir="$workspace/capture"
...
SCRIPT
    ...
    PACKAGE_VERSION="2026.03.24.120000+lifecycle" \
    bash "$REPO_DIR/scripts/build-pacman.sh" >/dev/null

    assert_file_exists "$install_file"
...
test_disabled_port_integration_cleanup_hooks_run_before_staging() {
    info "Checking disabled port integration cleanup hooks run before staging"
    local workspace="$TMP_DIR/port-integration-cleanup-hooks"
    local fake_root="$workspace/repo"
    local helper="$fake_root/scripts/lib/port-integrations.js"
    local cleanup_hook="$workspace/cleanup-hook.sh"
    local stage_hook="$workspace/stage-hook.sh"
    local order_log="$workspace/order.log"
    local app_dir="$workspace/official-app"
    local install_dir="$workspace/install"
    local work_dir="$workspace/work"

    mkdir -p "$fake_root/scripts/lib" "$app_dir" "$install_dir" "$work_dir"
    cat > "$helper" <<'NODE'
const fs = require("node:fs");
const command = process.argv[2];

if (command === "--cleanup-hooks") {
  process.stdout.write("disabled-fixture\t" + process.env.CLEANUP_HOOK + "\n");
} else if (command === "--stage-install") {
  fs.appendFileSync(process.env.HOOK_ORDER_LOG, "stage-install\n");
} else if (command === "--stage-hooks") {
  process.stdout.write("enabled-fixture\t" + process.env.STAGE_HOOK + "\n");
} else {
  process.exitCode = 2;
}
NODE
...
SCRIPT
    ...
    (
        export HOOK_ORDER_LOG="$order_log"
        export CLEANUP_HOOK="$cleanup_hook"
        export STAGE_HOOK="$stage_hook"
        export EXPECTED_APP_DIR="$app_dir"
        ...
        info() { :; }
        warn() { printf '%s\n' "$*" >&2; }
        # shellcheck source=scripts/lib/port-integrations.sh
        source "$REPO_DIR/scripts/lib/port-integrations.sh"
        run_port_integration_stage_hooks "$app_dir"
    )

    [ "$(cat "$order_log")" = $'cleanup\nstage-install\nstage' ] \
        || fail "Expected cleanup, declarative staging, then enabled stage hooks"
}
