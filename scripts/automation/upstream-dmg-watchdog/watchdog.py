...
def migrate_campaign(campaign: object) -> object:
    ...
    migrated.setdefault("integration_snapshot", migrated.get("integration_snapshot"))
...
class Store:
    def __init__(self, root: Path):
        ...
        self.snapshots = self.root / "integration-snapshots"
...
def command_worker_acquire(args: argparse.Namespace, store: Store) -> int:
    ...
    with store.locked():
        ...
        if previous_round is not None and repair_round is not previous_round:
            ...
            active["integration_snapshot"] = None
...
def integration_snapshot_helper(source: Path) -> dict:
    helper = Path(__file__).with_name("integration-snapshot.js")
    result = subprocess.run(
    ...
    if result.returncode != 0:
        raise RuntimeError(f"port integration snapshot failed: {result.stderr.strip()[:1000]}")
    try:
        value = json.loads(result.stdout)
    except json.JSONDecodeError as error:
        raise RuntimeError("port integration snapshot helper returned invalid JSON") from error
    if not isinstance(value, dict) or not isinstance(value.get("enabled"), list):
        raise RuntimeError("port integration snapshot helper returned an invalid contract")
    return value


def create_integration_snapshot(store: Store, campaign: dict, source: Path, now: float) -> dict:
    repair_round = current_round(campaign, create=True)
    snapshot_dir = store.snapshots / campaign["sha256"] / f"round-{repair_round['round']}"
    existing = campaign.get("integration_snapshot")
    if isinstance(existing, dict) and existing.get("snapshot_dir") == str(snapshot_dir) and snapshot_dir.is_dir():
        return existing

    description = integration_snapshot_helper(source)
    config = description.get("config") or {"enabled": []}
    enabled = description["enabled"]
    if config.get("enabled") != enabled:
        raise RuntimeError("normalized integration config does not match enabled integration manifests")
    ...
    try:
        if description.get("hasLocalConfig"):
            (temporary / "integrations.json").write_bytes(canonical_json(config))
            os.chmod(temporary / "integrations.json", 0o600)
        local_root = temporary / "local"
        for item in description.get("local") or []:
            if not isinstance(item, dict) or item.get("id") not in enabled:
                raise RuntimeError("port integration snapshot helper returned an invalid local integration")
            integration_id = item["id"]
            # Keep this lexical: the shared loader intentionally follows a
            # local integration directory symlink. copytree snapshots its resolved
            # contents while preserving supported links inside the tree.
            integration_dir = Path(os.path.abspath(str(item.get("dir") or "")))
            source_local_root = Path(os.path.abspath(source / "port-integrations" / "local"))
            if not integration_dir.is_relative_to(source_local_root) or not (integration_dir / "integration.json").is_file():
                raise RuntimeError(f"enabled local integration is invalid: {integration_id}")
            local_root.mkdir(mode=0o700, exist_ok=True)
            shutil.copytree(integration_dir, local_root / integration_id, symlinks=True)
            local_hashes[integration_id] = sha256_tree(local_root / integration_id)
    ...
    config_path = snapshot_dir / "integrations.json"
    snapshot = {
        ...
        "local_integration_tree_hashes": local_hashes,
        "created_at": utc_iso(now),
        "round": repair_round["round"],
    }
    campaign["integration_snapshot"] = snapshot
    repair_round["integration_snapshot"] = snapshot
    return snapshot


def materialize_integration_snapshot(snapshot: dict, worktree: Path) -> None:
    snapshot_dir = Path(snapshot["snapshot_dir"])
    target_integrations = worktree / "port-integrations"
    target_config = target_integrations / "integrations.json"
    target_local = target_integrations / "local"
    source_config = snapshot_dir / "integrations.json"
    source_local = snapshot_dir / "local"
    if source_config.is_file():
        temporary = target_integrations / f".integrations.{uuid.uuid4().hex}.json"
        try:
...
def command_sync_integrations(args: argparse.Namespace, store: Store) -> int:
    now = args.now if args.now is not None else time.time()
    with store.locked():
        state = store.load()
        ...
        target_integrations = worktree / "port-integrations"
        if not target_integrations.is_dir():
            raise RuntimeError("managed worktree has no port-integrations directory")
        snapshot = create_integration_snapshot(store, campaign, source, now)
        materialize_integration_snapshot(snapshot, worktree)
        campaign["updated_at"] = utc_iso(now)
        store.save(state)
        print("INTEGRATIONS_SYNCED " + (",".join(snapshot["enabled"]) if snapshot["enabled"] else "none"))
        return 0


def command_refresh_integration_snapshot(args: argparse.Namespace, store: Store) -> int:
    now = args.now if args.now is not None else time.time()
    with store.locked():
        state = store.load()
        require_owner(state, args.run_id, now)
        ...
        if campaign.get("acceptance_evidence"):
            raise RuntimeError("cannot refresh integrations after acceptance; start a new repair round")
        snapshot = campaign.pop("integration_snapshot", None)
        if isinstance(snapshot, dict) and snapshot.get("snapshot_dir"):
            snapshot_dir = Path(snapshot["snapshot_dir"])
            if snapshot_dir.is_relative_to(store.snapshots.resolve()):
                shutil.rmtree(snapshot_dir, ignore_errors=True)
        store.save(state)
        print("INTEGRATION_SNAPSHOT_CLEARED")
    return 0
...
def repair_scope(worktree: Path, base_sha: str | None, head_sha: str) -> dict:
    if not isinstance(base_sha, str) or not re.fullmatch(r"[0-9a-f]{40}", base_sha):
        return {"profile": "full", "paths": [], "integration_ids": [], "reason": "base-head-unavailable"}
    result = subprocess.run(
    ...
    if not paths:
        return {"profile": "unchanged", "paths": [], "integration_ids": []}
    integration_ids: set[str] = set()
    for path in paths:
        match = re.fullmatch(r"port-integrations/([^/]+)/.+", path)
        if not match or match.group(1) == "local":
            return {"profile": "full", "paths": paths, "integration_ids": []}
        integration_ids.add(match.group(1))
    return {"profile": "integration-only", "paths": paths, "integration_ids": sorted(integration_ids)}
...
def command_record_acceptance(args: argparse.Namespace, store: Store) -> int:
    ...
    with store.locked():
        ...
        snapshot = campaign.get("integration_snapshot")
        if not isinstance(snapshot, dict):
            raise RuntimeError("integration snapshot is missing")
        decision_integrations = (((decision.get("checks") or {}).get("patchReport") or {}).get("enabledIntegrations"))
        if decision_integrations != snapshot.get("enabled"):
            raise RuntimeError("acceptance decision enabled integrations do not match the snapshot")
        ...
        if config_hash != snapshot.get("config_sha256"):
            raise RuntimeError("integration snapshot config changed after creation")
        for integration_id, expected_hash in (snapshot.get("local_integration_tree_hashes") or {}).items():
            if sha256_tree(snapshot_dir / "local" / integration_id) != expected_hash:
                raise RuntimeError(f"local integration snapshot changed after creation: {integration_id}")
        ...
        evidence = {
            ...
            "integration_config_sha256": snapshot["config_sha256"],
            "local_integration_tree_hashes": snapshot.get("local_integration_tree_hashes") or {},
            "enabled_integrations": snapshot["enabled"],
...
def command_nix_preflight(args: argparse.Namespace, store: Store) -> int:
    ...
    with store.locked():
        ...
        if args.target and scope.get("profile") != "integration-only":
            raise RuntimeError("a focused Nix target is allowed only for a integration-only repair")
        target = args.target
        if scope.get("profile") == "integration-only" and not target:
            target = ".#checks.x86_64-linux.nix-port-integrations-multi-integration"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sync_integrations = subparsers.add_parser("sync-integrations")
    add_common(sync_integrations)
    sync_integrations.add_argument("--run-id", required=True)
    sync_integrations.add_argument("--source-checkout", required=True)
    sync_integrations.set_defaults(handler=command_sync_integrations)

    refresh_integrations = subparsers.add_parser("refresh-integration-snapshot")
    add_common(refresh_integrations)
    refresh_integrations.add_argument("--run-id", required=True)
    refresh_integrations.set_defaults(handler=command_refresh_integration_snapshot)
    ...
    preflight.add_argument(
        "--target",
        help="Verify only one Nix flake output for a focused integration-only repair",
    )
