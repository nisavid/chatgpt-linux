...
class WatchdogTests(unittest.TestCase):
    ...
    def prepare_acceptance_fixture(self, headers: Path):
        ...
        integration = worktree / "port-integrations" / "ui-tweaks"
        integration.mkdir(parents=True)
        (integration / "README.md").write_text("ui\n", encoding="utf-8")
        (integration / "integration.json").write_text('{"id":"ui-tweaks"}\n', encoding="utf-8")
        (worktree / ".gitignore").write_text("port-integrations/integrations.json\n", encoding="utf-8")
        ...
        (worktree / "port-integrations" / "integrations.json").write_text('{"enabled":["ui-tweaks"]}\n', encoding="utf-8")
        self.run_cli(
            "campaign-update", "--run-id", run_id, "--worktree", str(worktree), "--head-sha", head,
            "--base-sha", head, "--now", "1001",
        )
        self.run_cli(
            "sync-integrations", "--run-id", run_id, "--source-checkout", str(worktree), "--now", "1002"
        )
        report = worktree / "patch-report.json"
        report.write_text('{"enabledIntegrations":["ui-tweaks"],"patches":[]}\n', encoding="utf-8")
        decision = worktree / "decision.json"
        decision.write_text(json.dumps({
            "verdict": "accepted",
            "dmg": {"sha256": sha},
            "source": {"commit": head, "dirty": False},
            "checks": {"patchReport": {"enabledIntegrations": ["ui-tweaks"], "reportPath": str(report)}},
        }), encoding="utf-8")
    ...
    def test_completed_campaign_can_be_requeued_and_acquired_again(self):
        ...
        result = self.run_cli(
            "campaign-requeue", "--sha", sha, "--reason", "integration drift revalidation", "--now", "1100"
        )
        self.assertEqual(result.stdout.strip(), f"CHANGE_READY {sha}")
        state = self.load_state()
        self.assertEqual(state["active_campaign"]["sha256"], sha)
        self.assertEqual(state["active_campaign"]["requeue_reason"], "integration drift revalidation")
    ...
    def test_nix_repair_acquire_creates_a_new_round(self):
        ...
        state["active_campaign"]["integration_snapshot"] = {"round": 1}
        ...
        self.assertIsNone(self.load_state()["active_campaign"]["integration_snapshot"])
    ...
    def test_sync_integrations_copies_config_and_enabled_local_integration(self):
        headers = self.headers("a.headers", "etag-a", self.dmg_a)
        sha = hashlib.sha256(self.dmg_a.read_bytes()).hexdigest()
        self.probe(headers, self.dmg_a)
        acquired = self.run_cli("worker-acquire", "--sha", sha, "--now", "1000")
        ...
        source_integrations = source / "port-integrations"
        tracked_integration = source_integrations / "ui-tweaks"
        tracked_integration.mkdir(parents=True)
        (tracked_integration / "README.md").write_text("tracked\n", encoding="utf-8")
        (tracked_integration / "integration.json").write_text('{"id":"ui-tweaks"}\n', encoding="utf-8")
        local_integration = source_integrations / "local" / "private-integration"
        local_integration.mkdir(parents=True)
        (local_integration / "README.md").write_text("private\n", encoding="utf-8")
        (local_integration / "integration.json").write_text('{"id":"private-integration"}\n', encoding="utf-8")
        (local_integration / "payload.txt").write_text("payload\n", encoding="utf-8")
        (local_integration / "payload-link").symlink_to("payload.txt")
        config = {
            "enabled": ["ui-tweaks", "private-integration"],
            "settings": {"ui-tweaks": {"example": True}},
        }
        source_integrations.mkdir(exist_ok=True)
        (source_integrations / "integrations.json").write_text(json.dumps(config), encoding="utf-8")

        worktree = self.state / "worktrees" / sha[:12]
        target_integrations = worktree / "port-integrations"
        (target_integrations / "ui-tweaks").mkdir(parents=True)
        self.run_cli(
            "campaign-update", "--run-id", run_id, "--worktree", str(worktree), "--now", "1001"
        )
        result = self.run_cli(
            "sync-integrations", "--run-id", run_id, "--source-checkout", str(source), "--now", "1002"
        )
        self.assertEqual(result.stdout.strip(), "INTEGRATIONS_SYNCED ui-tweaks,private-integration")
        copied = json.loads((target_integrations / "integrations.json").read_text(encoding="utf-8"))
        normalized_config = {**config, "disabled": []}
        self.assertEqual(copied, normalized_config)
        self.assertEqual(
            (target_integrations / "local" / "private-integration" / "README.md").read_text(encoding="utf-8"),
            "private\n",
        )
        snapshot = self.load_state()["active_campaign"]["integration_snapshot"]
        self.assertEqual(snapshot["enabled"], ["ui-tweaks", "private-integration"])
        self.assertTrue((target_integrations / "local" / "private-integration" / "payload-link").is_symlink())
        self.assertIn("private-integration", snapshot["local_integration_tree_hashes"])
        (source_integrations / "integrations.json").write_text('{"enabled":[]}\n', encoding="utf-8")
        (local_integration / "payload.txt").write_text("changed\n", encoding="utf-8")
        self.run_cli(
            "sync-integrations", "--run-id", run_id, "--source-checkout", str(source), "--now", "1003"
        )
        self.assertEqual(json.loads((target_integrations / "integrations.json").read_text()), normalized_config)
        self.assertEqual((target_integrations / "local" / "private-integration" / "payload.txt").read_text(), "payload\n")

    def test_sync_integrations_normalizes_legacy_alias_and_rejects_missing_manifest(self):
        headers = self.headers("a.headers", "etag-a", self.dmg_a)
        sha = hashlib.sha256(self.dmg_a.read_bytes()).hexdigest()
        self.probe(headers, self.dmg_a)
        acquired = self.run_cli("worker-acquire", "--sha", sha, "--now", "1000")
        ...
        integration = source / "port-integrations" / "open-target-discovery"
        integration.mkdir(parents=True)
        (integration / "README.md").write_text("integration\n", encoding="utf-8")
        (integration / "integration.json").write_text('{"id":"open-target-discovery"}\n', encoding="utf-8")
        (source / "port-integrations" / "integrations.json").write_text(
            '{"enabled":["zed-opener"],"settings":{"zed-opener":{"value":1}}}\n', encoding="utf-8"
        )
        worktree = self.state / "worktrees" / sha[:12]
        target = worktree / "port-integrations"
        (target / "open-target-discovery").mkdir(parents=True)
        self.run_cli("campaign-update", "--run-id", run_id, "--worktree", str(worktree), "--now", "1001")
        self.run_cli("sync-integrations", "--run-id", run_id, "--source-checkout", str(source), "--now", "1002")
        copied = json.loads((target / "integrations.json").read_text())
        self.assertEqual(copied["enabled"], ["open-target-discovery"])
        self.assertEqual(copied["settings"]["open-target-discovery"], {"value": 1})

        self.run_cli("refresh-integration-snapshot", "--run-id", run_id, "--now", "1003")
        missing = source / "port-integrations" / "local" / "private-missing"
        missing.mkdir(parents=True)
        (missing / "README.md").write_text("missing\n", encoding="utf-8")
        (source / "port-integrations" / "integrations.json").write_text('{"enabled":["private-missing"]}\n', encoding="utf-8")
        result = self.run_cli(
            "sync-integrations", "--run-id", run_id, "--source-checkout", str(source), "--now", "1004", check=False
        )
        self.assertEqual(result.returncode, 5)
        self.assertIn("Enabled port integration ids not found", result.stderr)

    def test_sync_integrations_snapshots_a_symlinked_local_integration_directory(self):
        headers = self.headers("a.headers", "etag-a", self.dmg_a)
        sha = hashlib.sha256(self.dmg_a.read_bytes()).hexdigest()
        self.probe(headers, self.dmg_a)
        acquired = self.run_cli("worker-acquire", "--sha", sha, "--now", "1000")
        ...
        local_root = source / "port-integrations" / "local"
        local_root.mkdir(parents=True)
        external = self.root / "external-private-integration"
        external.mkdir()
        (external / "README.md").write_text("private\n", encoding="utf-8")
        (external / "integration.json").write_text('{"id":"private-integration"}\n', encoding="utf-8")
        (external / "payload.txt").write_text("immutable\n", encoding="utf-8")
        (local_root / "private-integration").symlink_to(external, target_is_directory=True)
        (source / "port-integrations" / "integrations.json").write_text(
            '{"enabled":["private-integration"]}\n', encoding="utf-8"
        )

        worktree = self.state / "worktrees" / sha[:12]
        (worktree / "port-integrations").mkdir(parents=True)
        self.run_cli("campaign-update", "--run-id", run_id, "--worktree", str(worktree), "--now", "1001")
        ...
        self.assertEqual(result.stdout.strip(), "INTEGRATIONS_SYNCED private-integration")
        copied = worktree / "port-integrations" / "local" / "private-integration"
        self.assertTrue(copied.is_dir())
        self.assertFalse(copied.is_symlink())
        self.assertEqual((copied / "payload.txt").read_text(), "immutable\n")

    def test_sync_integrations_rejects_unmanaged_worktree(self):
        headers = self.headers("a.headers", "etag-a", self.dmg_a)
        sha = hashlib.sha256(self.dmg_a.read_bytes()).hexdigest()
        self.probe(headers, self.dmg_a)
        acquired = self.run_cli("worker-acquire", "--sha", sha, "--now", "1000")
        ...
        (outside / "port-integrations").mkdir(parents=True)
        self.run_cli(
            "campaign-update", "--run-id", run_id, "--worktree", str(outside), "--now", "1001"
        )
        result = self.run_cli(
            "sync-integrations", "--run-id", run_id, "--source-checkout", str(self.root),
            "--now", "1002", check=False,
        )
        self.assertEqual(result.returncode, 5)
        self.assertIn("outside the managed worktree root", result.stderr)

    def test_record_acceptance_binds_dmg_integrations_and_git_head(self):
        headers = self.headers("a.headers", "etag-a", self.dmg_a)
        ...
        self.assertEqual(evidence["enabled_integrations"], ["ui-tweaks"])
        self.assertEqual(evidence["validation_scope"]["profile"], "unchanged")
        self.assertTrue(Path(evidence["patch_report_path"]).is_file())

    def test_record_acceptance_classifies_integration_only_repair_scope(self):
        headers = self.headers("a.headers", "etag-a", self.dmg_a)
        _, run_id, worktree, base, decision = self.prepare_acceptance_fixture(headers)
        (worktree / "port-integrations" / "ui-tweaks" / "README.md").write_text("updated ui\n", encoding="utf-8")
        subprocess.run([
            "git", "-C", str(worktree), "add", "port-integrations/ui-tweaks/README.md",
        ], check=True)
        subprocess.run(["git", "-C", str(worktree), "commit", "-m", "integration repair"], check=True, stdout=subprocess.DEVNULL)
        head = subprocess.check_output(["git", "-C", str(worktree), "rev-parse", "HEAD"], text=True).strip()
        ...
        self.assertEqual(scope["profile"], "integration-only")
        self.assertEqual(scope["integration_ids"], ["ui-tweaks"])
        self.assertEqual(scope["paths"], ["port-integrations/ui-tweaks/README.md"])
    ...
    def test_empty_pr_rollup_uses_dispatched_ci_run_and_returns_failure_to_worker(self):
        ...
        scenario, env = self.write_scenario({
            ...
            "run_logs": {"77": "frameless-titlebar enabled-integration-drift"},
        })
        result = self.nix_probe(headers, env, now=1100)
    ...
    def test_dry_run_nix_source_failure_second_round_and_successful_campaign_completion(self):
        ...
        failure_scenario, failure_env = self.write_scenario({
            ...
            "run_logs": {"501": "enabled-integration-drift"},
        })
