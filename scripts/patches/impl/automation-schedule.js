...
const AUTOMATION_SCHEDULE_PATCH_MARKER = "function chatgptLinuxNormalizeRruleNumbers(";
...
function automationScheduleReplacement(block) {
  return [
    ...
    `function ${block.timeOfDayFn}(e){let t=chatgptLinuxNormalizeRruleNumbers(e.byhour,0,23),n=chatgptLinuxNormalizeRruleNumbers(e.byminute,0,59);return e.dtstart&&(t.length!==0||(t=[e.dtstart.getHours()]),n.length!==0||(n=[e.dtstart.getMinutes()])),t.length!==0&&n.length!==0?{hours:t,minutes:n}:null}`,
    `function ${block.singleNumberFn}(e){let t=chatgptLinuxNormalizeRruleNumbers(e,-9007199254740991,9007199254740991);return t.length>0?t[0]:null}`,
    "function chatgptLinuxNormalizeRruleNumbers(e,t,n){let r=Array.isArray(e)?e:[e];return Array.from(new Set(r.filter(e=>typeof e==`number`&&Number.isInteger(e)&&e>=t&&e<=n))).sort((e,t)=>e-t)}",
    "function chatgptLinuxRruleTimes(e){let t=[];for(let n of e.hours)for(let r of e.minutes)t.push(n*60+r);return Array.from(new Set(t)).sort((e,t)=>e-t)}",
    `function ${block.dailyFn}(e,t){return ${block.weeklyFn}(e,t,[])}`,
    `function ${block.weeklyFn}(e,t,n){let r=new Date(e),i=r.getDay(),a=n.length>0?n:[0,1,2,3,4,5,6],o=chatgptLinuxRruleTimes(t);if(o.length===0)return e;for(let t=0;t<=7;t+=1){let n=(i+t)%7;if(!a.includes(n))continue;for(let n of o){let i=Math.floor(n/60),a=n%60,s=new Date(r.getFullYear(),r.getMonth(),r.getDate()+t,i,a,0,0);if(s.getTime()>e)return s.getTime()}}return e}`,
...
function applyAutomationScheduleMultiTimePatch(source) {
  if (source.includes("function chatgptLinuxNormalizeRruleNumbers(")) {
    return source;
  }
...
function applyGenericAutomationScheduleMultiTimePatch(source) {
  ...
  const helperPatch =
    helperBlock +
    "function chatgptLinuxNormalizeRruleNumbers(e,t,n){let r=Array.isArray(e)?e:[e];return Array.from(new Set(r.filter(e=>typeof e==`number`&&Number.isInteger(e)&&e>=t&&e<=n))).sort((e,t)=>e-t)}" +
    "function chatgptLinuxRruleTimes(e,t,n){let r=chatgptLinuxNormalizeRruleNumbers(e,0,23),i=chatgptLinuxNormalizeRruleNumbers(t,0,59);n.dtstart&&(r.length!==0||(r=[n.dtstart.getHours()]),i.length!==0||(i=[n.dtstart.getMinutes()]));let a=[];for(let e of r)for(let t of i)a.push(" +
    combineFn +
    "(e,t));return Array.from(new Set(a)).sort()}" +
    "function chatgptLinuxAutomationTimeLabel(e,t){let n=Array.isArray(e.timeValues)&&e.timeValues.length>0?e.timeValues:[e.time],r=n.map(e=>" +
    labelFn +
    "(e,t)).filter(Boolean);return r.length===0?null:typeof t.formatList==`function`?t.formatList(r,{type:`conjunction`}):r.join(`, `)}";

  const parserPatch =
    "hasMultipleTimeValues:chatgptLinuxRruleTimes(r.byhour,r.byminute,r).length>1,interval:Math.max(1,Math.round(r.interval??1)),minute:a,origOptions:n.origOptions,rruleText:e,time:" +
    timeFn +
    "(r.byhour,r.byminute,r),timeValues:chatgptLinuxRruleTimes(r.byhour,r.byminute,r),weekdays:i";

  const summaryPatch = summaryBlock
    .replace("if(!e||e.hasMultipleTimeValues)return null;", "if(!e)return null;")
    .replace(
      "let i=" + labelFn + "(e.time,t);",
      "let i=chatgptLinuxAutomationTimeLabel(e,t);",
    );
...
function applyCurrentAutomationScheduleMultiTimePatch(source) {
  ...
  const helperPatch =
    `${helperNeedle}function chatgptLinuxNormalizeRruleNumbers(e,t,n){let r=Array.isArray(e)?e:[e];return Array.from(new Set(r.filter(e=>typeof e==\`number\`&&Number.isInteger(e)&&e>=t&&e<=n))).sort((e,t)=>e-t)}function chatgptLinuxRruleTimes(e,t,n){let r=chatgptLinuxNormalizeRruleNumbers(e,0,23),i=chatgptLinuxNormalizeRruleNumbers(t,0,59);n.dtstart&&(r.length!==0||(r=[n.dtstart.getHours()]),i.length!==0||(i=[n.dtstart.getMinutes()]));let a=[];for(let e of r)for(let t of i)a.push(Mn(e,t));return Array.from(new Set(a)).sort()}function chatgptLinuxAutomationTimeLabel(e,t){let n=Array.isArray(e.timeValues)&&e.timeValues.length>0?e.timeValues:[e.time],r=n.map(e=>Mt(e,t)).filter(Boolean);return r.length===0?null:typeof t.formatList==\`function\`?t.formatList(r,{type:\`conjunction\`}):r.join(\`, \`)}`;
  ...
  const parserPatch =
    "hasMultipleTimeValues:chatgptLinuxRruleTimes(r.byhour,r.byminute,r).length>1,interval:Math.max(1,Math.round(r.interval??1)),minute:a,origOptions:n.origOptions,rruleText:e,time:Tn(r.byhour,r.byminute,r),timeValues:chatgptLinuxRruleTimes(r.byhour,r.byminute,r),weekdays:i";
  ...
  const summaryPatch =
    "function bn(e,t){if(!e)return null;let n=on(e.weekdays),r=n.length===q.length;if(e.freq===K.MINUTELY)return Sn({intervalMinutes:e.interval,intl:t,isEveryDay:r,weekdays:n});if(e.freq===K.HOURLY)return xn({intervalHours:e.interval,intl:t,isEveryDay:r,weekdays:n});if(e.freq!==K.DAILY&&e.freq!==K.WEEKLY)return null;let i=chatgptLinuxAutomationTimeLabel(e,t);return i?wn({intl:t,isEveryDay:r,timeLabel:i,weekdays:n}):null}";
