...
const CI_POLICY_REQUIRED_OFFICIAL_DMG = "required-official-dmg";
...
const CI_POLICIES = new Set([
  CI_POLICY_REQUIRED_OFFICIAL_DMG,
  CI_POLICY_REQUIRED_UPSTREAM,
  CI_POLICY_OPTIONAL,
  CI_POLICY_OPT_IN,
]);

function normalizeCiPolicy(ciPolicy) {
  return ciPolicy === CI_POLICY_REQUIRED_UPSTREAM
    ? CI_POLICY_REQUIRED_OFFICIAL_DMG
    : ciPolicy;
}
...
function assertDescriptorBase(descriptor, phase) {
  ...
  const ciPolicy = normalizeCiPolicy(descriptor.ciPolicy ?? CI_POLICY_OPTIONAL);
...
function patchDescriptor(phase, descriptor) {
  ...
  return {
    ...
    phase,
    ciPolicy: normalizeCiPolicy(descriptor.ciPolicy ?? CI_POLICY_OPTIONAL),
  };
}
