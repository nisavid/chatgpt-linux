...
const LINUX_SETTINGS_PATCH_SYMBOL_PATTERN = /\bchatgptLinux[A-Za-z0-9_$]*SettingsIcon\b/g;
...
function patchFindingsBySurface(patchReport, surfacesById = {}) {
  ...
  for (const patch of patchReport?.patches ?? []) {
    ...
    const patchText = [patch.name, patch.reason, patch.integrationId, patch.featureId].filter(Boolean).join(" ");
