...
const { enabledIntegrationFailuresFromReport, optionalDriftFromReport } = require("./patch-report.js");
...
function evaluateUpstreamDmg(options) {
  ...
  if (core.report) {
    const enabledIntegrationFailures = profile.rejectEnabledIntegrationDrift
      ? enabledIntegrationFailuresFromReport(core.report)
      : [];
    const enabledIntegrationFailureNames = new Set(enabledIntegrationFailures.map((failure) => failure.name));
    ...
    blockers.push(...enabledIntegrationFailures.map((failure) => ({
      code: "enabled-integration-drift",
      check: `integration:${failure.integrationId}`,
      name: failure.name,
      status: failure.status,
      reason: failure.reason ?? `Enabled integration ${failure.integrationId} did not apply cleanly`,
    })));
    warnings.push(...driftWarnings("core", core.report, enabledIntegrationFailureNames));
  } else {
  ...
  return {
    ...
    checks: {
      ...
      patchReport: {
        ...
        enabledIntegrations: Array.isArray(core.report?.enabledIntegrations)
          ? core.report.enabledIntegrations
          : Array.isArray(core.report?.enabledFeatures)
            ? core.report.enabledFeatures
            : [],
      },
    },
    blockers,
