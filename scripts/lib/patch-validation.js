...
function validatePatchReport(report, profile, requirements = {}) {
  ...
  const requiredEnabledIntegrations = uniqueStrings(requirements.requiredEnabledIntegrations ?? []);
  ...
  for (const integrationId of requiredEnabledIntegrations) {
    if (!enabledIntegrations.has(integrationId)) {
      failures.push(`integration ${integrationId}: not enabled in patch report`);
    }
  }
