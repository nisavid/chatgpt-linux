...
node_toolchain_compatible() {
    ...
    if ! node_runtime_compatible "$runtime_dir/bin/node"; then
        return 1
    fi
    if [ ! -x "$runtime_dir/bin/npm" ]; then
        return 1
    fi
    if [ ! -x "$runtime_dir/bin/npx" ]; then
        return 1
    fi
...
ensure_managed_node_runtime() {
    ...
    if probe_node_runtime node_toolchain_compatible "$destination_dir"; then
    ...
    do
        ...
        if probe_node_runtime copy_node_runtime "$source_dir" "$destination_dir"; then
