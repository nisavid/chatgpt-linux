...
`update-chatgpt-hash.yml`; the campaign must first reach `awaiting-nix` after an
...
3. Run `sync-integrations` once with the user's primary checkout as
   `--source-checkout`, not the managed repair worktree. It uses the repository port integration loader,
   ...
   snapshot. Use `refresh-integration-snapshot` only before acceptance when an
...
5. `record-acceptance` requires an accepted verdict for the campaign DMG, a
   clean matching source commit, the exact enabled port integration snapshot, and a full
...
8. Wait for these exact gates: Rust and Smoke Tests, Debian, RPM, Pacman, Nix
   Package Builds, and Build App Against Official DMG. Run
...
### Port-integration-only fast path

A round is port-integration-only only when every changed tracked path is inside the
affected `port-integrations/<id>/` directories. For that scope:

1. Run the affected port integration tests, not every repository integration test.
2. Rebuild and record acceptance against the exact campaign DMG and immutable
   port integration snapshot.
...
4. Publish the repair PR as soon as those focused checks pass. The normal CI
   workflow and official-DMG build provide the six merge gates in parallel.

Do not run the full local `ci-local.sh pr`, Debian, RPM, or pacman matrix for a
port-integration-only repair. Those broad local checks are required when a round changes
core patches, the shared port integration loader, installer, updater, packaging, or a
mixture of integration and shared paths. Reuse the immutable DMG download, integration
...
Only the bot-authored `chatgpt/nix-upstream-refresh` PR is eligible for automatic
...
The status view is an allowlisted operational projection. Failed-workflow logs
and raw error diagnostics remain ephemeral and are never written to watchdog
state or echoed by this command; legacy log excerpts are removed on the next
state save.
