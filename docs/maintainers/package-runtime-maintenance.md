...
- the updater crate, binary, service, config, state, cache, and logs use
  `codex-app-updater`;
- incoming `codex-desktop` and `codex-update-manager` names should not appear
  in durable package/runtime surfaces unless they are compatibility metadata
  such as `provides` or `conflicts`.
...
- `Codex.dmg` is the cached official OpenAI Codex DMG input.
