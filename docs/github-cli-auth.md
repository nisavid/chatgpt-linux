...
Then ask ChatGPT to run the same command in a shell. If the terminal shows
...
# Electron/app-scoped XDG paths inherited by ChatGPT.
...
Then use the wrapper anywhere ChatGPT requires authenticated GitHub CLI
access:
...
Then retry `~/.local/bin/gh-normal auth status` inside ChatGPT.
