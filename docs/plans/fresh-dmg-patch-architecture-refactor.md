...
- Run `node --check scripts/patch-linux-window-ui.js scripts/patches/*.js scripts/patches/lib/*.js scripts/lib/port-integrations.js`.
- Run `node --test scripts/patches/*.test.js scripts/patches/**/*.test.js scripts/patch-linux-window-ui.test.js`.
- Run `node --test port-integrations/*/test.js`.
...
- Update `port-integrations/README.md` and `docs/port-integrations-architecture.md` to document `patchDescriptors` as the only patch entrypoint.
...
- Current repo Port Integrations must still work after being migrated to the new contract.
