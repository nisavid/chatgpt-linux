...
pub fn preflight(
    ...
    let requested_path = resolve_runtime_cli_path(config, state, explicit_cli_path.as_deref())?;
    preflight_selected(state, paths, requested_path, allow_install_missing)
}
...
fn reconcile_if_present_with_requested_path(
    ...
    Ok(preflight_selected(state, paths, requested_path, false)?.updated)
}
...
fn read_installed_version(cli_path: &Path) -> Result<String> {
    ...
    extract_version(&fallback).ok_or_else(|| {
        anyhow!(
            ...
            redaction::redact_for_persistence(fallback.trim())
        )
    })
}
...
"#,
    ...
    fn preflight_preserves_homebrew_channel_for_cached_canonical_path() -> Result<()> {
        ...
        let mut state = PersistedState::new(true);
        let first = preflight_selected(&mut state, &paths, Some(visible_codex.clone()), false)?;
        ...
        let second = preflight_selected(&mut state, &paths, cached_cli_path, false)?;
    ...
    #[test]
    fn group_writable_standalone_cli_is_accepted_by_preflight_selected() -> Result<()> {
        let _env_guard = env_lock();
        let temp = tempdir()?;
        let paths = test_runtime_paths(temp.path());
        paths.ensure_dirs()?;
        ...
        let outcome = preflight_selected(&mut state, &paths, Some(visible_codex), false)?;
    ...
    #[test]
    fn group_writable_standalone_cli_ancestor_is_accepted_by_preflight_selected() -> Result<()> {
        let _env_guard = env_lock();
        let temp = tempdir()?;
        let paths = test_runtime_paths(temp.path());
        paths.ensure_dirs()?;
    ...
    fn standalone_cli_symlink_updates_with_standalone_installer() -> Result<()> {
        ...
        let mut state = PersistedState::new(true);
        state.cli_path = Some(visible_codex.clone());
        let outcome = preflight_selected(&mut state, &paths, Some(visible_codex.clone()), false)?;
    ...
    fn failing_standalone_cli_update_reports_standalone_installer_error() -> Result<()> {
        ...
        let mut state = PersistedState::new(true);
        state.cli_path = Some(visible_codex.clone());
        let error = preflight_selected(&mut state, &paths, Some(visible_codex), false)
...
"#,
        ...
        let outcome =
            preflight_selected(&mut state, &paths, Some(fixture.visible_cli.clone()), false)?;
    ...
    fn preflight_does_not_repair_unknown_executable() -> Result<()> {
        ...
        let mut state = PersistedState::new(true);
        let error = preflight_selected(&mut state, &paths, Some(codex_path.clone()), true)
...
"#,
    ...
    fn failed_missing_cli_install_persists_failed_status() -> Result<()> {
        ...
        let mut state = PersistedState::new(true);
        let error = preflight_selected(&mut state, &paths, None, true)
    ...
    fn pending_repair_blocks_missing_cli_registry_and_install_transitions() -> Result<()> {
        ...
        state.cli_installed_version = Some("0.42.0".to_string());
        preflight_selected(&mut state, &paths, None, true)
...
"#,
        ...
        let outcome = preflight_selected(&mut state, &paths, Some(system_codex.clone()), false)?;
    ...
    fn preflight_uses_configured_cli_path() -> Result<()> {
        ...
        let paths = test_runtime_paths(temp.path());
        paths.ensure_dirs()?;
        let _restore = configure_cli_test_env(temp.path(), [temp.path().join("missing-bin")])?;
