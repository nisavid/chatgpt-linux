...
const HELP: &str = "chatgpt-computer-use-cosmic\n\nUsage:\n  chatgpt-computer-use-cosmic probe\n  chatgpt-computer-use-cosmic list-windows\n  chatgpt-computer-use-cosmic focused-window\n  chatgpt-computer-use-cosmic activate-window --window-id <id>";
...
fn activation_state_path() -> PathBuf {
    if let Some(runtime_dir) = std::env::var_os("XDG_RUNTIME_DIR") {
        PathBuf::from(runtime_dir).join("chatgpt-computer-use-cosmic-last-activation.json")
    } else {
        std::env::temp_dir().join("chatgpt-computer-use-cosmic-last-activation.json")
    }
}
