...
This integration is bundle-native. It installs a small Rust reaper plus runtime
triggers:
...
The integration does not hardcode local tools or providers.

## Compatibility

This integration can be enabled together with `node-repl-reaper`. It does not wrap
...
Add to `port-integrations/integrations.json`:

```json
{ "enabled": ["mcp-helper-reaper"] }
```

then rebuild/reinstall. The integration is disabled by default.
...
removes staged launcher hooks and binaries, and removes this integration's
...
```bash
rtk cargo test --manifest-path port-integrations/mcp-helper-reaper/reaper/Cargo.toml
node --test port-integrations/mcp-helper-reaper/test.js
```
