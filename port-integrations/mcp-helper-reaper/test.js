...
test("stage hook finds cargo in HOME cargo bin when PATH omits it", () => {
  ...
  const integrationDir = path.join(scriptRoot, "port-integrations", "mcp-helper-reaper");
  const reaperCrateDir = path.join(integrationDir, "reaper");
  ...
  for (const file of [
    ...
    fs.copyFileSync(path.join(FEATURE_DIR, file), path.join(integrationDir, file));
  }
  writeExecutable(nodeRepl, "#!/usr/bin/env bash\necho original node_repl\n");
  writeExecutable(
