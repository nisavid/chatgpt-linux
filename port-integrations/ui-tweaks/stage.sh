...
const { loadEnabledPortIntegrations } = require(path.join(scriptDir, "scripts/lib/port-integrations.js"));
const { dockIconEnabled } = require(path.join(
  scriptDir,
  "port-integrations/ui-tweaks/patches/dock-icon.js",
));
const integration = loadEnabledPortIntegrations().find(({ id }) => id === "ui-tweaks");
process.stdout.write(integration != null && dockIconEnabled({ integration }) ? "true" : "false");
NODE
)"
