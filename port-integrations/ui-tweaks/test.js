...
function copyIntegrationTo(integrationsRoot) {
  const integrationDir = path.join(integrationsRoot, "ui-tweaks");
  fs.mkdirSync(integrationDir, { recursive: true });
  for (const name of ["integration.json", "README.md", "patch.js"]) {
    fs.copyFileSync(path.join(__dirname, name), path.join(integrationDir, name));
  }
  fs.cpSync(path.join(__dirname, "patches"), path.join(integrationDir, "patches"), { recursive: true });
}
...
test("ui-tweaks is discoverable, default-on, and explicitly disableable", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "ui-tweaks-feature-"));
  try {
    const integrationsRoot = path.join(tempDir, "port-integrations");
    fs.mkdirSync(integrationsRoot, { recursive: true });
    copyIntegrationTo(integrationsRoot);
    fs.writeFileSync(path.join(integrationsRoot, "integrations.example.json"), '{"enabled":[]}\n');

    const manifests = discoverPortIntegrationManifests({ integrationsRoot });
    assert.equal(manifests.length, 1);
    assert.equal(manifests[0].id, "ui-tweaks");
    assert.equal(manifests[0].manifest.defaultEnabled, true);
    assert.ok(loadPortIntegrationPatchDescriptors({ integrationsRoot }).length > 0);

    fs.writeFileSync(path.join(integrationsRoot, "integrations.json"), '{"disabled":["ui-tweaks"]}\n');
    assert.deepEqual(loadPortIntegrationPatchDescriptors({ integrationsRoot }), []);

    fs.writeFileSync(path.join(integrationsRoot, "integrations.json"), '{"enabled":["ui-tweaks"]}\n');
    const descriptors = loadPortIntegrationPatchDescriptors({ integrationsRoot });
    assert.deepEqual(
      descriptors.map((descriptor) => [descriptor.id, descriptor.phase, descriptor.ciPolicy]),
      [
        ["integration:ui-tweaks:sidebar-project-name-style", "webview-asset", "optional"],
        ["integration:ui-tweaks:model-picker-default-advanced-view", "webview-asset", "optional"],
        ["integration:ui-tweaks:model-picker-inline-model-list", "webview-asset", "optional"],
        [
          "integration:ui-tweaks:model-picker-dynamic-supported-reasoning-efforts",
          "webview-asset",
          "optional",
        ],
        ["integration:ui-tweaks:reasoning-effort-labels-english", "webview-asset", "optional"],
        ["integration:ui-tweaks:appearance-dock-icon-main-process", "main-bundle", "optional"],
        ["integration:ui-tweaks:appearance-dock-icon-settings-row", "webview-asset", "optional"],
        ["integration:ui-tweaks:appearance-dock-icon-settings-search", "webview-asset", "optional"],
        ["integration:ui-tweaks:home-suggested-prompts-main-process", "main-bundle", "optional"],
        ["integration:ui-tweaks:home-suggested-prompts-app-page", "webview-asset", "optional"],
        ["integration:ui-tweaks:home-suggested-prompts-settings-row", "webview-asset", "optional"],
        ["integration:ui-tweaks:home-suggested-prompts-content", "webview-asset", "optional"],
      ],
    );
  } finally {
...
test("model picker tweak can be disabled through integration settings", () => {
  const stateSource = modelPickerStateBundleFixture();
  const menuSource = modelPickerMenuBundleFixture();
  const context = {
    integration: {
...
test("integration manifest defaults reach descriptor context through the integration loader", () => {
...
test("default project name style is bold without changing fixed row geometry", () => {
  const integrationJson = JSON.parse(fs.readFileSync(path.join(__dirname, "integration.json"), "utf8"));
  assert.equal(integrationJson.tweaks.sidebar.projectName.style, DEFAULT_PROJECT_NAME_STYLE);
...
test("integration settings override the tracked defaults through integrations.json", () => {
...
test("invalid integration settings warn and fall back to defaults", () => {
  ...
  try {
    ...
    assert.match(warnings.join("\n"), /WARN: port integration 'ui-tweaks' settings/);
    assert.match(patched, /font-weight: 700 !important;/);
    assert.doesNotMatch(patched, /padding-top/);
  } finally {
    fs.rmSync(tempDir, { recursive: true, force: true });
