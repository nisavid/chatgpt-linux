...
function modelPickerConfig(context) {
  const defaults = context?.integration?.manifest?.tweaks?.modelPicker?.showModelsByDefault;
  const settings = context?.integration?.settings?.tweaks?.modelPicker?.showModelsByDefault;
  return {
