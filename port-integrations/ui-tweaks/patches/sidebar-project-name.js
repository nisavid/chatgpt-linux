...
const STYLE_ID = "chatgpt-linux-ui-tweaks-sidebar-project-name-style";
const RUNTIME_MARKER = "chatgptLinuxUiTweaksSidebarProjectNameStyleRuntime";
...
function sidebarProjectNameConfig(context) {
  const defaults = context?.integration?.manifest?.tweaks?.sidebar?.projectName;
  const settings = context?.integration?.settings?.tweaks?.sidebar?.projectName;
  return {
