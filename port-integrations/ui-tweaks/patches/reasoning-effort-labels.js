...
const ENGLISH_REASONING_LABELS = Object.freeze({
  ...
  "composer.mode.local.reasoning.low.label.v2": "Low",
...
function reasoningLabelConfig(context) {
  const defaults = context?.integration?.manifest?.tweaks?.reasoning?.keepEffortLabelsEnglish;
  const settings = context?.integration?.settings?.tweaks?.reasoning?.keepEffortLabelsEnglish;
  return {
