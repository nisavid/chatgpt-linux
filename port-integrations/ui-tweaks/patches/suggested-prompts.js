...
function suggestedPromptsConfig(context) {
  ...
  const defaults = integration?.manifest?.tweaks?.home?.suggestedPrompts;
  const settings = integration?.settings?.tweaks?.home?.suggestedPrompts;
  return {
  ...
  try {
    ...
    return source.replace(
      ...
      (_match, sourceFlag, debugOverride) =>
        `${sourceFlag}=${debugOverride}===\`curated\`&&function ${HOME_CONTENT_SOURCE_MARKER}(){return!0}()`,
    );
  } catch (error) {
    console.warn(
