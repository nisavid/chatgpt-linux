...
function copyIntegrationTo(integrationsRoot) {
  const integrationDir = path.join(integrationsRoot, "frameless-titlebar");
...
test("frameless-titlebar stays disabled until listed in integrations.json", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "frameless-titlebar-integration-"));
  try {
    ...
    fs.writeFileSync(path.join(integrationsRoot, "integrations.json"), '{"enabled":["frameless-titlebar"]}\n');
    const descriptors = loadPortIntegrationPatchDescriptors({ integrationsRoot });
    assert.deepEqual(
      descriptors.map((descriptor) => descriptor.id).sort(),
      [
        "integration:frameless-titlebar:main-process",
        "integration:frameless-titlebar:webview-window-controls-layout",
      ],
    );
    const webviewPatch = descriptors.find(
      (descriptor) => descriptor.id === "integration:frameless-titlebar:webview-window-controls-layout",
    );
