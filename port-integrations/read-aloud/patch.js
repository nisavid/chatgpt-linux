...
const SETTINGS_KEY = "chatgpt-linux-read-aloud-enabled";
const KOKORO_MODEL_KEY = "chatgpt-linux-read-aloud-kokoro-model";
const KOKORO_PYTHON_KEY = "chatgpt-linux-read-aloud-kokoro-python";
const KOKORO_SPEED_KEY = "chatgpt-linux-read-aloud-kokoro-speed";
const KOKORO_VOICES_KEY = "chatgpt-linux-read-aloud-kokoro-voices";
...
const HELPER_MARKER = "chatgptLinuxReadAloudClick";
const SETUP_MARKER = "chatgptLinuxReadAloudSetup";
...
const CURRENT_SETTINGS_BLOCK_MARKER = "chatgptLinuxReadAloudSettingsAliasesV2";
const GENERAL_SETTINGS_ROW_CALL = "(0,$.jsx)(chatgptLinuxReadAloudSettingsRow,{})";
...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    `function chatgptLinuxReadAloudCleanText(e){return typeof e!==\`string\`?\`\`:e.replace(/\\r\\n/g,\`\\n\`).replace(new RegExp(\`\\\`\\\`\\\`[\\\\s\\\\S]*?\\\`\\\`\\\`\`,\`gu\`),\` code block. \`).replace(/\\[([^\\]]+)\\]\\(([^)]+)\\)/gu,\`$1\`).replace(/[*_#>~]/gu,\`\`).replace(/\\n{3,}/gu,\`\\n\\n\`).trim().slice(0,8e3)}`,
    `function chatgptLinuxReadAloudHasHebrew(e){return /[\\u0590-\\u05ff]/u.test(e)}`,
    `function chatgptLinuxReadAloudHome(){return process.env.HOME||process.env.USERPROFILE||${osVar}.homedir?.()||\`\`}`,
    `function chatgptLinuxReadAloudDataHome(){let e=chatgptLinuxReadAloudHome(),t=process.env.XDG_DATA_HOME||e&&${pathVar}.join(e,\`.local\`,\`share\`);return t||\`\`}`,
    `function chatgptLinuxReadAloudSettingsAppId(){let e=process.env.CHATGPT_LINUX_APP_ID||process.env.CHATGPT_APP_ID||\`chatgpt\`;return/^[A-Za-z0-9._-]+$/.test(e)?e:\`chatgpt\`}`,
    `function chatgptLinuxReadAloudSettingsPath(){let e=process.env.CHATGPT_LINUX_SETTINGS_FILE;if(typeof e===\`string\`&&e.length>0)return e;let t=chatgptLinuxReadAloudHome(),n=process.env.XDG_CONFIG_HOME||t&&${pathVar}.join(t,\`.config\`);return n?${pathVar}.join(n,chatgptLinuxReadAloudSettingsAppId(),\`settings.json\`):null}`,
    `function chatgptLinuxReadAloudSettings(){try{let e=chatgptLinuxReadAloudSettingsPath();if(!e||!${fsVar}.existsSync(e))return{};let t=JSON.parse(${fsVar}.readFileSync(e,\`utf8\`));return t&&typeof t===\`object\`&&!Array.isArray(t)?t:{}}catch{return{}}}`,
    `function chatgptLinuxReadAloudWriteSettings(e){let t=chatgptLinuxReadAloudSettingsPath();if(!t)throw Error(\`settings path unavailable\`);${fsVar}.mkdirSync(${pathVar}.dirname(t),{recursive:!0});${fsVar}.writeFileSync(t,JSON.stringify(e,null,2)+\`\\n\`)}`,
    `function chatgptLinuxReadAloudEnabled(){if(process.env.CHATGPT_LINUX_READ_ALOUD_ENABLED===\`1\`)return!0;let e=chatgptLinuxReadAloudSettings();return e["chatgpt-linux-read-aloud-enabled"]===!0}`,
    `function chatgptLinuxReadAloudFileExists(e){try{return!!e&&${fsVar}.existsSync(e)}catch{return!1}}`,
    `function chatgptLinuxReadAloudCommandExists(command){if(!command)return!1;if(command.includes(\`/\`))try{return!!command&&${fsVar}.existsSync(command)&&(${fsVar}.accessSync(command,${fsVar}.constants.X_OK),!0)}catch{return!1};try{return ${childProcessVar}.spawnSync(\`which\`,[command],{stdio:\`ignore\`}).status===0}catch{return!1}}`,
    `function chatgptLinuxReadAloudNativeFallbackEnabled(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_NATIVE_FALLBACK?.trim().toLowerCase();return!(e===\`0\`||e===\`false\`||e===\`off\`||e===\`no\`)}`,
    `function chatgptLinuxReadAloudNativeFallbackAvailable(){return chatgptLinuxReadAloudNativeFallbackEnabled()&&(chatgptLinuxReadAloudCommandExists(\`spd-say\`)||chatgptLinuxReadAloudCommandExists(\`espeak-ng\`))}`,
    `function chatgptLinuxReadAloudAudioEnv(){if(process.env.XDG_RUNTIME_DIR)return{};let e;try{e=process.getuid?.()}catch{}if(e==null)return{};let t=\`/run/user/\${e}\`;try{if(${fsVar}.existsSync(${pathVar}.join(t,\`pipewire-0\`))||${fsVar}.existsSync(${pathVar}.join(t,\`pulse\`,\`native\`)))return{XDG_RUNTIME_DIR:t}}catch{}return{}}`,
    `function chatgptLinuxReadAloudKokoroRunner(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_RUNNER?.trim();if(e)return e;let t=process.resourcesPath?${pathVar}.join(process.resourcesPath,\`read-aloud\`,\`kokoro-stdin\`):\`\`;return t&&chatgptLinuxReadAloudCommandExists(t)?t:\`kokoro-stdin\`}`,
    `function chatgptLinuxReadAloudKokoroPython(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_PYTHON?.trim(),t=chatgptLinuxReadAloudSettings()[${serializeJavaScriptValue(KOKORO_PYTHON_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(chatgptLinuxReadAloudDataHome(),\`chatgpt\`,\`read-aloud\`,\`kokoro-venv\`,\`bin\`,\`python\`)}`,
    `function chatgptLinuxReadAloudKokoroModel(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_MODEL?.trim(),t=chatgptLinuxReadAloudSettings()[${serializeJavaScriptValue(KOKORO_MODEL_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(chatgptLinuxReadAloudDataHome(),\`kokoro\`,\`kokoro-v1.0.onnx\`)}`,
    `function chatgptLinuxReadAloudKokoroVoices(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICES?.trim(),t=chatgptLinuxReadAloudSettings()[${serializeJavaScriptValue(KOKORO_VOICES_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(chatgptLinuxReadAloudDataHome(),\`kokoro\`,\`voices-v1.0.bin\`)}`,
    `function chatgptLinuxReadAloudKokoroVoice(){return process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICE?.trim()||\`bm_george\`}`,
    `function chatgptLinuxReadAloudClampSpeed(e){let t=Number(e);return Number.isFinite(t)?Math.min(1.4,Math.max(.7,Math.round(t*20)/20)):1.05}`,
    `function chatgptLinuxReadAloudKokoroSpeed(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_SPEED?.trim(),t=chatgptLinuxReadAloudSettings()["chatgpt-linux-read-aloud-kokoro-speed"];return chatgptLinuxReadAloudClampSpeed(e!==void 0&&e!==\`\`?e:t??1.05)}`,
    `function chatgptLinuxReadAloudKokoroMissing(){let e=[];chatgptLinuxReadAloudCommandExists(chatgptLinuxReadAloudKokoroRunner())||e.push(\`runner\`);chatgptLinuxReadAloudCommandExists(chatgptLinuxReadAloudKokoroPython())||e.push(\`python\`);chatgptLinuxReadAloudFileExists(chatgptLinuxReadAloudKokoroModel())||e.push(\`model\`);chatgptLinuxReadAloudFileExists(chatgptLinuxReadAloudKokoroVoices())||e.push(\`voices\`);chatgptLinuxReadAloudCommandExists(\`aplay\`)||e.push(\`aplay\`);return e}`,
    `function chatgptLinuxReadAloudKokoroModelUrl(){return process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_MODEL_URL?.trim()||${serializeJavaScriptValue(KOKORO_MODEL_URL)}}`,
    `function chatgptLinuxReadAloudKokoroVoicesUrl(){return process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICES_URL?.trim()||${serializeJavaScriptValue(KOKORO_VOICES_URL)}}`,
    `function chatgptLinuxReadAloudConfig(){let e=chatgptLinuxReadAloudKokoroMissing();return{enabled:chatgptLinuxReadAloudEnabled(),engine:(process.env.CHATGPT_LINUX_READ_ALOUD_ENGINE?.trim()||\`kokoro\`).toLowerCase(),nativeFallback:chatgptLinuxReadAloudNativeFallbackEnabled(),customCommand:!!process.env.CHATGPT_LINUX_READ_ALOUD_COMMAND?.trim(),kokoro:{available:e.length===0,missing:e,voice:chatgptLinuxReadAloudKokoroVoice(),speed:chatgptLinuxReadAloudKokoroSpeed(),python:chatgptLinuxReadAloudKokoroPython(),model:chatgptLinuxReadAloudKokoroModel(),voices:chatgptLinuxReadAloudKokoroVoices(),modelUrl:chatgptLinuxReadAloudKokoroModelUrl(),voicesUrl:chatgptLinuxReadAloudKokoroVoicesUrl()}}}`,
    `function chatgptLinuxReadAloudBackendReady(e){let t=process.env.CHATGPT_LINUX_READ_ALOUD_COMMAND?.trim();return!!(e?.kokoro?.available||chatgptLinuxReadAloudNativeFallbackAvailable()||t&&chatgptLinuxReadAloudCommandExists(t))}`,
    `function chatgptLinuxReadAloudSetupResult(){let e=chatgptLinuxReadAloudConfig();return chatgptLinuxReadAloudBackendReady(e)?{ok:!0,config:e}:{ok:!1,reason:\`voice-unavailable\`,config:e}}`,
    `function chatgptLinuxReadAloudRun(command,args,timeoutMs=3e5){return new Promise((resolve,reject)=>{let child,timer;try{child=${childProcessVar}.spawn(command,args,{stdio:\`ignore\`,windowsHide:!0,env:{...process.env,...chatgptLinuxReadAloudAudioEnv()}}),timer=setTimeout(()=>{try{child.kill(\`SIGTERM\`)}catch{}reject(Error(\`command timed out\`))},timeoutMs),timer.unref?.(),child.on(\`error\`,error=>{clearTimeout(timer),reject(error)}),child.on(\`close\`,code=>{clearTimeout(timer),code===0?resolve():reject(Error(\`command failed\`))})}catch(error){clearTimeout(timer),reject(error)}})}`,
    `function chatgptLinuxReadAloudPythonOk(pythonBin){try{return ${childProcessVar}.spawnSync(pythonBin,[\`-c\`,\`import sys; raise SystemExit(0 if (3,10) <= sys.version_info < (3,14) else 1)\`],{stdio:\`ignore\`,env:{...process.env,...chatgptLinuxReadAloudAudioEnv()}}).status===0}catch{return!1}}`,
    `function chatgptLinuxReadAloudFindPython(){for(let e of [process.env.PYTHON?.trim(),\`python3.12\`,\`python3.13\`,\`python3.11\`,\`python3.10\`,\`python3\`])if(e&&chatgptLinuxReadAloudCommandExists(e)&&chatgptLinuxReadAloudPythonOk(e))return e;return null}`,
    `async function chatgptLinuxReadAloudInstallRuntime(){let e=chatgptLinuxReadAloudKokoroPython();if(chatgptLinuxReadAloudCommandExists(e))return;let t=${pathVar}.dirname(${pathVar}.dirname(e)),n=chatgptLinuxReadAloudFindPython();if(!n)throw Error(\`Python 3.10-3.13 is required for Kokoro\`);${fsVar}.mkdirSync(${pathVar}.dirname(t),{recursive:!0});if(chatgptLinuxReadAloudCommandExists(\`uv\`)){await chatgptLinuxReadAloudRun(\`uv\`,[\`venv\`,\`--python\`,n,t]);await chatgptLinuxReadAloudRun(\`uv\`,[\`pip\`,\`install\`,\`--python\`,e,\`kokoro-onnx>=0.5.0\`,\`numpy>=2.0.2\`],6e5);return}await chatgptLinuxReadAloudRun(n,[\`-m\`,\`venv\`,t]);await chatgptLinuxReadAloudRun(e,[\`-m\`,\`ensurepip\`,\`--upgrade\`]);await chatgptLinuxReadAloudRun(e,[\`-m\`,\`pip\`,\`install\`,\`--upgrade\`,\`pip\`],6e5);await chatgptLinuxReadAloudRun(e,[\`-m\`,\`pip\`,\`install\`,\`kokoro-onnx>=0.5.0\`,\`numpy>=2.0.2\`],6e5)}`,
    `function chatgptLinuxReadAloudDownloadFile(url,target,minBytes,redirects=0){return new Promise((resolve,reject)=>{if(redirects>5){reject(Error(\`too many redirects\`));return}let parsed;try{parsed=new URL(url)}catch{reject(Error(\`invalid download URL\`));return}let get=parsed.protocol===\`https:\`?require(\`node:https\`).get:parsed.protocol===\`http:\`?require(\`node:http\`).get:null;if(!get){reject(Error(\`unsupported download URL\`));return}${fsVar}.mkdirSync(${pathVar}.dirname(target),{recursive:!0});let partial=\`\${target}.part\`,bytes=0,done=!1;try{${fsVar}.rmSync(partial,{force:!0})}catch{}let cleanup=()=>{try{${fsVar}.rmSync(partial,{force:!0})}catch{}};let fail=e=>{if(done)return;done=!0;cleanup();reject(e instanceof Error?e:Error(String(e)))};let fileStream=${fsVar}.createWriteStream(partial),request=get(parsed,{headers:{"User-Agent":\`chatgpt-read-aloud\`}},response=>{if(response.statusCode>=300&&response.statusCode<400&&response.headers.location){response.resume?.();fileStream.close(()=>{});cleanup();chatgptLinuxReadAloudDownloadFile(new URL(response.headers.location,parsed).toString(),target,minBytes,redirects+1).then(resolve,reject);return}if(response.statusCode!==200){response.resume?.();fileStream.close(()=>{});fail(Error(\`download failed with status \${response.statusCode}\`));return}response.on(\`data\`,chunk=>{bytes+=chunk.length}),response.pipe(fileStream),fileStream.on(\`finish\`,()=>fileStream.close(()=>{if(done)return;if(bytes<minBytes){fail(Error(\`download too small\`));return}try{${fsVar}.renameSync(partial,target),done=!0,resolve()}catch(error){fail(error)}}))});request.on(\`error\`,fail),fileStream.on(\`error\`,fail)})}`,
    `async function chatgptLinuxReadAloudDownloadKokoro(){let e=chatgptLinuxReadAloudKokoroModel(),t=chatgptLinuxReadAloudKokoroVoices();chatgptLinuxReadAloudFileExists(e)||await chatgptLinuxReadAloudDownloadFile(chatgptLinuxReadAloudKokoroModelUrl(),e,5e7);chatgptLinuxReadAloudFileExists(t)||await chatgptLinuxReadAloudDownloadFile(chatgptLinuxReadAloudKokoroVoicesUrl(),t,1e6);let n=chatgptLinuxReadAloudSettings();n[${serializeJavaScriptValue(KOKORO_PYTHON_KEY)}]=chatgptLinuxReadAloudKokoroPython(),n[${serializeJavaScriptValue(KOKORO_MODEL_KEY)}]=e,n[${serializeJavaScriptValue(KOKORO_VOICES_KEY)}]=t,chatgptLinuxReadAloudWriteSettings(n)}`,
    `async function chatgptLinuxReadAloudChooseModelDir(){let electronApi;try{let loadElectron=()=>require(\`electron\`),electronModule=loadElectron();electronApi=typeof chatgptLinuxPatchExternalOpen===\`function\`?chatgptLinuxPatchExternalOpen(electronModule):electronModule}catch{return{ok:!1,reason:\`dialog-unavailable\`}}let result=await electronApi.dialog.showOpenDialog({title:\`Choose Kokoro model folder\`,properties:[\`openDirectory\`]});if(result.canceled||!result.filePaths?.[0])return{ok:!1,reason:\`cancelled\`};let dir=result.filePaths[0],modelPath=${pathVar}.join(dir,\`kokoro-v1.0.onnx\`),voicesPath=${pathVar}.join(dir,\`voices-v1.0.bin\`);if(!chatgptLinuxReadAloudFileExists(modelPath)||!chatgptLinuxReadAloudFileExists(voicesPath))return{ok:!1,reason:\`missing-files\`,path:dir};let settings=chatgptLinuxReadAloudSettings();settings[${serializeJavaScriptValue(KOKORO_MODEL_KEY)}]=modelPath,settings[${serializeJavaScriptValue(KOKORO_VOICES_KEY)}]=voicesPath,chatgptLinuxReadAloudWriteSettings(settings);return chatgptLinuxReadAloudSetupResult()}`,
    `async function chatgptLinuxReadAloudSetup(e={}){if(process.platform!==\`linux\`)return{ok:!1,reason:\`not-linux\`};try{if(e.mode===\`choose-folder\`)return await chatgptLinuxReadAloudChooseModelDir();if(e.mode===\`download\`){await chatgptLinuxReadAloudInstallRuntime();await chatgptLinuxReadAloudDownloadKokoro();return chatgptLinuxReadAloudSetupResult()}return{ok:!1,reason:\`unknown-mode\`}}catch(t){let n=t?.message??String(t);return{ok:!1,reason:n.includes(\`Python 3.10-3.13\`)?\`python-unavailable\`:\`setup-failed\`,message:n}}}`,
    `function chatgptLinuxReadAloudReport(e){try{console.info(\`[linux-read-aloud] \${JSON.stringify(e)}\`)}catch{}return e}`,
    `let chatgptLinuxReadAloudProc=null;function chatgptLinuxReadAloudStop(){let e=chatgptLinuxReadAloudProc;chatgptLinuxReadAloudProc=null;if(!e)return chatgptLinuxReadAloudReport({stopped:!1,reason:\`idle\`});try{e.pid&&process.kill(-e.pid,\`SIGTERM\`)}catch{try{e.kill?.(\`SIGTERM\`)}catch{}}return chatgptLinuxReadAloudReport({stopped:!0})}`,
    `function chatgptLinuxReadAloudSpawn(command,args,options={}){if(!chatgptLinuxReadAloudCommandExists(command))return!1;try{chatgptLinuxReadAloudStop();let child=${childProcessVar}.spawn(command,args,{...options,stdio:options.stdio??\`ignore\`,windowsHide:!0,detached:!0,env:{...process.env,...chatgptLinuxReadAloudAudioEnv(),...(options.env||{})}});chatgptLinuxReadAloudProc=child,child.on?.(\`exit\`,()=>{chatgptLinuxReadAloudProc===child&&(chatgptLinuxReadAloudProc=null)}),child.unref?.();return!0}catch{return!1}}`,
    `function chatgptLinuxReadAloudSpawnStdin(command,args,text,extraEnv={}){if(!chatgptLinuxReadAloudCommandExists(command))return!1;try{chatgptLinuxReadAloudStop();let child=${childProcessVar}.spawn(command,args,{stdio:[\`pipe\`,\`ignore\`,\`ignore\`],windowsHide:!0,detached:!0,env:{...process.env,...chatgptLinuxReadAloudAudioEnv(),...extraEnv}});chatgptLinuxReadAloudProc=child,child.on?.(\`exit\`,()=>{chatgptLinuxReadAloudProc===child&&(chatgptLinuxReadAloudProc=null)}),child.stdin?.end(text),child.unref?.();return!0}catch{return!1}}`,
    `function chatgptLinuxReadAloudKokoro(e){let t=chatgptLinuxReadAloudKokoroMissing();if(t.length)return{spoken:!1,reason:\`kokoro-unavailable\`,missing:t};let n=chatgptLinuxReadAloudKokoroSpeed(),r={CHATGPT_LINUX_READ_ALOUD_KOKORO_PYTHON:chatgptLinuxReadAloudKokoroPython(),CHATGPT_LINUX_READ_ALOUD_KOKORO_MODEL:chatgptLinuxReadAloudKokoroModel(),CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICES:chatgptLinuxReadAloudKokoroVoices(),CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICE:chatgptLinuxReadAloudKokoroVoice(),CHATGPT_LINUX_READ_ALOUD_KOKORO_SPEED:String(n)};return chatgptLinuxReadAloudSpawnStdin(chatgptLinuxReadAloudKokoroRunner(),[],e,r)?{spoken:!0,engine:\`kokoro\`,voice:chatgptLinuxReadAloudKokoroVoice(),speed:n}:{spoken:!1,reason:\`kokoro-spawn-failed\`}}`,
    `function chatgptLinuxReadAloudPiper(text,modelPath){let piperBin=process.env.CHATGPT_LINUX_READ_ALOUD_PIPER_BIN?.trim()||\`piper\`;if(!modelPath||!${fsVar}.existsSync(modelPath)||!chatgptLinuxReadAloudCommandExists(piperBin)||!chatgptLinuxReadAloudCommandExists(\`aplay\`))return!1;try{chatgptLinuxReadAloudStop();let piperOptions={stdio:[\`pipe\`,\`pipe\`,\`ignore\`],windowsHide:!0,detached:!0,env:{...process.env,...chatgptLinuxReadAloudAudioEnv()}},piper=${childProcessVar}.spawn(piperBin,[\`--model\`,modelPath,\`--output-raw\`],piperOptions),aplay=${childProcessVar}.spawn(\`aplay\`,[\`-q\`,\`-r\`,\`22050\`,\`-c\`,\`1\`,\`-f\`,\`S16_LE\`,\`-t\`,\`raw\`],{stdio:[\`pipe\`,\`ignore\`,\`ignore\`],windowsHide:!0,detached:!0,env:{...process.env,...chatgptLinuxReadAloudAudioEnv()}});chatgptLinuxReadAloudProc=piper,piper.on?.(\`exit\`,()=>{chatgptLinuxReadAloudProc===piper&&(chatgptLinuxReadAloudProc=null)}),piper.stdout?.pipe(aplay.stdin),piper.stdin?.end(text),piper.unref?.(),aplay.unref?.();return!0}catch{return!1}}`,
    `function chatgptLinuxReadAloudSpeak(input,options={}){if(process.platform!==\`linux\`)return chatgptLinuxReadAloudReport({spoken:!1,reason:\`not-linux\`});if(options?.requireEnabled!==!1&&!chatgptLinuxReadAloudEnabled())return chatgptLinuxReadAloudReport({spoken:!1,reason:\`disabled\`});let text=chatgptLinuxReadAloudCleanText(input);if(!text)return chatgptLinuxReadAloudReport({spoken:!1,reason:\`empty\`});let customCommand=process.env.CHATGPT_LINUX_READ_ALOUD_COMMAND?.trim();if(customCommand&&chatgptLinuxReadAloudSpawnStdin(customCommand,[],text))return chatgptLinuxReadAloudReport({spoken:!0,engine:\`custom\`});let engine=(process.env.CHATGPT_LINUX_READ_ALOUD_ENGINE?.trim()||\`kokoro\`).toLowerCase(),hasHebrew=chatgptLinuxReadAloudHasHebrew(text),piperModel=process.env.CHATGPT_LINUX_READ_ALOUD_PIPER_MODEL?.trim();if(engine===\`piper\`)return chatgptLinuxReadAloudReport(chatgptLinuxReadAloudPiper(text,piperModel)?{spoken:!0,engine:\`piper\`}:{spoken:!1,reason:\`piper-unavailable\`});let kokoroResult=chatgptLinuxReadAloudKokoro(text);if(kokoroResult.spoken)return chatgptLinuxReadAloudReport(kokoroResult);if(!chatgptLinuxReadAloudNativeFallbackEnabled())return chatgptLinuxReadAloudReport(kokoroResult);if(chatgptLinuxReadAloudPiper(text,piperModel))return chatgptLinuxReadAloudReport({spoken:!0,engine:\`piper\`});let voice=process.env.CHATGPT_LINUX_READ_ALOUD_VOICE?.trim(),rate=process.env.CHATGPT_LINUX_READ_ALOUD_RATE?.trim()||\`-10\`,voiceType=process.env.CHATGPT_LINUX_READ_ALOUD_VOICE_TYPE?.trim();try{chatgptLinuxReadAloudCommandExists(\`spd-say\`)&&${childProcessVar}.spawn(\`spd-say\`,[\`-C\`],{stdio:\`ignore\`,windowsHide:!0,env:{...process.env,...chatgptLinuxReadAloudAudioEnv()}}).unref?.()}catch{}let spdArgs=[\`-r\`,rate,...(voiceType?[\`-t\`,voiceType]:[]),...(voice?[\`-y\`,voice]:[]),\`-l\`,hasHebrew?\`he\`:\`en\`,\`--\`,text];if(chatgptLinuxReadAloudSpawn(\`spd-say\`,spdArgs))return chatgptLinuxReadAloudReport({spoken:!0,engine:\`spd-say\`});let espeakVoice=voice||(hasHebrew?\`he\`:\`en-us\`);return chatgptLinuxReadAloudReport(chatgptLinuxReadAloudSpawn(\`espeak-ng\`,[\`-v\`,espeakVoice,\`-s\`,process.env.CHATGPT_LINUX_READ_ALOUD_ESPEAK_RATE?.trim()||\`165\`,\`--\`,text])?{spoken:!0,engine:\`espeak-ng\`}:kokoroResult)}`,
    `function chatgptLinuxReadAloudHandle(e={}){return e.action===\`config\`?chatgptLinuxReadAloudConfig():e.action===\`setup\`?chatgptLinuxReadAloudSetup(e):e.action===\`stop\`?chatgptLinuxReadAloudStop():e.action===\`speak\`&&e.source===\`button\`?chatgptLinuxReadAloudSpeak(e.text,{requireEnabled:!1}):chatgptLinuxReadAloudReport({spoken:!1,reason:\`not-explicit\`})}`,
  ].join("");

  const handler = `"${HANDLER_NAME}":async(e)=>chatgptLinuxReadAloudHandle(e),`;
...
function readAloudRuntimeSource() {
  return [
    `;(()=>{const VERSION=${serializeJavaScriptValue(RUNTIME_VERSION)};if(globalThis.chatgptLinuxReadAloudVersion===VERSION)return;globalThis.chatgptLinuxReadAloudVersion=VERSION;try{globalThis.speechSynthesis?.cancel?.()}catch{}`,
    `const METHOD=${serializeJavaScriptValue(HANDLER_NAME)};let seq=0,pending=new Map,currentButton=null,currentSpeakTimer=null;`,
    ...
    `function setButton(button,state,label){if(!button)return;let title=buttonLabel(state,label);button.dataset.chatgptLinuxReadAloudState=state;button.title=title;button.setAttribute("aria-label",title);button.disabled=state==="loading"}`,
    ...
    `async function setup(mode,button){let original=button?.dataset.chatgptLinuxReadAloudOriginalLabel||button?.textContent||"";if(button&&!button.dataset.chatgptLinuxReadAloudOriginalLabel)button.dataset.chatgptLinuxReadAloudOriginalLabel=original;try{button&&(button.disabled=!0,button.textContent=mode==="download"?"Downloading...":"Choosing...");let result=await post({action:"setup",mode},mode==="download"?9e5:6e4).then(e=>e.body).catch(()=>({ok:!1,reason:"request-failed"}));button&&(button.textContent=setupLabel(result));setTimeout(()=>{button&&(button.textContent=original,button.disabled=!1)},1800);return result}catch{button&&(button.textContent="Setup failed",setTimeout(()=>{button.textContent=original,button.disabled=!1},1800))}}`,
    `function installStyle(){if(document.getElementById("chatgpt-linux-read-aloud-style"))return;let e=document.createElement("style");e.id="chatgpt-linux-read-aloud-style";e.textContent=".chatgpt-linux-read-aloud-row{display:flex;align-items:center;margin-top:4px}.chatgpt-linux-read-aloud-button{width:28px;height:24px;display:inline-flex;align-items:center;justify-content:center;border:1px solid var(--token-border);background:transparent;color:var(--text-secondary,var(--token-description-foreground));border-radius:6px;padding:0;cursor:pointer}.chatgpt-linux-read-aloud-icon{width:15px;height:15px}.chatgpt-linux-read-aloud-button:hover{background:var(--token-list-hover-background,rgba(127,127,127,.12));color:var(--text-primary,var(--token-foreground))}.chatgpt-linux-read-aloud-button:disabled{opacity:.65;cursor:default}.chatgpt-linux-read-aloud-button[data-chatgpt-linux-read-aloud-state=speaking]{background:var(--token-list-hover-background,rgba(127,127,127,.14));color:var(--text-primary,var(--token-foreground))}.chatgpt-linux-read-aloud-button[data-chatgpt-linux-read-aloud-state=error]{color:var(--token-error-foreground,#c00);border-color:currentColor}";document.head.appendChild(e)}`,
...
function applySettingsPatch(source) {
  return source
    .replace(`,readAloud:"chatgpt-linux-read-aloud-enabled"`, "")
    .replace(
...
function linuxDesktopReadAloudSettingsSource() {
  return [
    `function chatgptLinuxReadAloudPaceValue(e){let t=Number(e);return Number.isFinite(t)?Math.min(1.4,Math.max(.7,Math.round(t*20)/20)):1.05}`,
...
function replaceLinuxDesktopReadAloudSettings(source) {
  ...
  const paceStart = source.lastIndexOf("function chatgptLinuxReadAloudPaceValue(", classStart);
...
function applyLinuxDesktopSettingsPatch(source) {
  ...
  if (!patched.includes(`readAloud:${serializeJavaScriptValue(SETTINGS_KEY)}`)) {
    const keyNeedle = `autoUpdateOnExit:${serializeJavaScriptValue(linuxSettingsKeys.autoUpdateOnExit)}`;
    const keyBoundary = `${keyNeedle}};function chatgptLinuxChecked`;
    ...
    patched = patched.replace(
      keyBoundary,
      `${keyNeedle},readAloud:${serializeJavaScriptValue(SETTINGS_KEY)},readAloudSpeed:${serializeJavaScriptValue(KOKORO_SPEED_KEY)}};function chatgptLinuxChecked`,
    );
  }
...
function currentSettingsAliasesNeedRefresh(source) {
  ...
  return !(
    ...
    source.includes("chatgptLinuxReadAloudChooseFolderLabel=c.formatMessage") &&
...
function currentGeneralSettingsReadAloudBlockSource(source) {
  ...
  return `/*${CURRENT_SETTINGS_BLOCK_MARKER}*/function chatgptLinuxReadAloudPaceValue(e){let t=Number(e);return Number.isFinite(t)?Math.min(1.4,Math.max(.7,Math.round(t*20)/20)):1.05}function chatgptLinuxReadAloudSettingsRow(){let[e,t]=(0,${react}.useState)(!1),[n,r]=(0,${react}.useState)(1.05),[i,a]=(0,${react}.useState)(!0),[o,s]=(0,${react}.useState)(null),c=${formatHook}();(0,${react}.useEffect)(()=>{let e=!0;return a(!0),Promise.all([${postGlobalState}(\`get-global-state\`,{params:{key:${serializeJavaScriptValue(SETTINGS_KEY)}}}),${postGlobalState}(\`get-global-state\`,{params:{key:${serializeJavaScriptValue(KOKORO_SPEED_KEY)}}})]).then(([n,i])=>{e&&(t(n?.value===!0),r(chatgptLinuxReadAloudPaceValue(i?.value??1.05)),s(null))}).catch(t=>{e&&s(t instanceof Error?t.message:String(t))}).finally(()=>{e&&a(!1)}),()=>{e=!1}},[]);let l=(0,${jsx}.jsx)(${formattedMessage},{id:\`settings.general.readAloud.label\`,defaultMessage:\`Read aloud responses\`,description:\`Label for Linux read aloud setting\`}),u=(0,${jsx}.jsx)(${formattedMessage},{id:\`settings.general.readAloud.description\`,defaultMessage:\`Show a read aloud button under assistant responses. If the Kokoro voice files are missing, choose a local folder or download them.\`,description:\`Description for Linux read aloud setting\`}),d=(0,${jsx}.jsx)(${formattedMessage},{id:\`settings.general.readAloud.pace.label\`,defaultMessage:\`Speech pace\`,description:\`Label for Linux read aloud pace setting\`}),f=(0,${jsx}.jsx)(${formattedMessage},{id:\`settings.general.readAloud.pace.description\`,defaultMessage:\`Adjust the read aloud speed\`,description:\`Description for Linux read aloud pace setting\`}),chatgptLinuxReadAloudToggleLabel=c.formatMessage({id:\`settings.general.readAloud.label\`,defaultMessage:\`Read aloud responses\`,description:\`Label for Linux read aloud setting\`}),chatgptLinuxReadAloudChooseFolderLabel=c.formatMessage({id:\`settings.general.readAloud.chooseFolder\`,defaultMessage:\`Choose folder\`,description:\`Button label for choosing an existing Kokoro model folder\`}),chatgptLinuxReadAloudDownloadVoiceLabel=c.formatMessage({id:\`settings.general.readAloud.downloadVoice\`,defaultMessage:\`Download voice\`,description:\`Button label for downloading the Kokoro voice model\`}),chatgptLinuxReadAloudPaceLabel=c.formatMessage({id:\`settings.general.readAloud.pace.label\`,defaultMessage:\`Speech pace\`,description:\`Label for Linux read aloud pace setting\`}),chatgptLinuxReadAloudHelpLabel=c.formatMessage({id:\`settings.general.readAloud.help\`,defaultMessage:\`Choose folder expects kokoro-v1.0.onnx and voices-v1.0.bin. Download voice creates a managed Python runtime and downloads the Kokoro files from Hugging Face.\`,description:\`Help text for Linux read aloud setup actions\`}),v=e=>{let n=e;t(n),s(null),${postGlobalState}(\`set-global-state\`,{params:{key:${serializeJavaScriptValue(SETTINGS_KEY)},value:n}}).catch(e=>{t(!n),s(e instanceof Error?e.message:String(e))})},y=e=>{let o=n,t=chatgptLinuxReadAloudPaceValue(e.currentTarget.value);r(t),s(null),${postGlobalState}(\`set-global-state\`,{params:{key:${serializeJavaScriptValue(KOKORO_SPEED_KEY)},value:t}}).catch(e=>{r(o),s(e instanceof Error?e.message:String(e))})},b=\`rounded-md border border-token-border px-2 py-1 text-sm text-token-text-primary hover:bg-token-surface-secondary disabled:opacity-60\`,x=(0,${jsx}.jsxs)(\`div\`,{className:\`flex items-center justify-end gap-2\`,children:[(0,${jsx}.jsx)(\`input\`,{type:\`range\`,min:.7,max:1.4,step:.05,value:n,disabled:i,onChange:y,"aria-label":chatgptLinuxReadAloudPaceLabel,className:\`h-2 w-36 accent-token-text-primary\`}),(0,${jsx}.jsx)(\`span\`,{className:\`w-12 text-right text-sm text-token-text-secondary\`,children:\`\${n.toFixed(2)}x\`})]}),C=o?(0,${jsx}.jsx)(\`div\`,{className:\`text-sm text-token-text-secondary\`,children:o}):null;return(0,${jsx}.jsxs)(${jsx}.Fragment,{children:[(0,${jsx}.jsx)(${settingsRow},{label:l,description:(0,${jsx}.jsxs)(${jsx}.Fragment,{children:[u,C]}),control:(0,${jsx}.jsxs)(\`div\`,{className:\`flex flex-wrap items-center justify-end gap-2\`,children:[(0,${jsx}.jsx)(${toggle},{checked:e===!0,disabled:i,onChange:v,ariaLabel:chatgptLinuxReadAloudToggleLabel}),e?(0,${jsx}.jsxs)(\`div\`,{className:\`flex flex-wrap items-center justify-end gap-2\`,children:[(0,${jsx}.jsx)(\`button\`,{type:\`button\`,className:b,onClick:e=>globalThis.${SETUP_MARKER}?.(\`choose-folder\`,e.currentTarget),children:chatgptLinuxReadAloudChooseFolderLabel}),(0,${jsx}.jsx)(\`button\`,{type:\`button\`,className:b,onClick:e=>globalThis.${SETUP_MARKER}?.(\`download\`,e.currentTarget),children:chatgptLinuxReadAloudDownloadVoiceLabel}),(0,${jsx}.jsx)(\`span\`,{className:\`inline-flex h-7 w-7 select-none items-center justify-center rounded-full border border-token-border text-sm text-token-text-secondary\`,title:chatgptLinuxReadAloudHelpLabel,"aria-label":chatgptLinuxReadAloudHelpLabel,children:\`?\`})]}):null]})}),e?(0,${jsx}.jsx)(${settingsRow},{label:d,description:f,control:x}):null]})}function chatgptLinuxReadAloudSettingsPage(){return(0,${jsx}.jsx)(${settingsPage},{title:(0,${jsx}.jsx)(${formattedMessage},{id:\`settings.readAloud.title\`,defaultMessage:\`Read Aloud\`,description:\`Title for Linux read aloud settings section\`}),subtitle:(0,${jsx}.jsx)(${formattedMessage},{id:\`settings.readAloud.subtitle\`,defaultMessage:\`Listen to assistant responses with a local Kokoro voice.\`,description:\`Subtitle for Linux read aloud settings section\`}),children:(0,${jsx}.jsx)(\`div\`,{className:\`max-w-3xl\`,children:(0,${jsx}.jsx)(chatgptLinuxReadAloudSettingsRow,{})})})}`;
}
...
function replaceExistingGeneralSettingsReadAloudRow(source, functionName, blockSource) {
  const rowStart = source.indexOf("function chatgptLinuxReadAloudSettingsRow(){");
  if (rowStart === -1) {
    return source;
  }
  const paceStart = source.indexOf("function chatgptLinuxReadAloudPaceValue(");
...
function applyGeneralSettingsPatch(source) {
  ...
  if (patched.includes(SETTINGS_KEY)) {
    const needsCurrentAliasRefresh =
      functionName !== "Gn" &&
      patched.includes("function chatgptLinuxReadAloudSettingsPage") &&
...
function applyGeneralSettingsExportPatch(source) {
  if (source.includes("chatgptLinuxReadAloudSettingsPage as ReadAloudSettings")) {
    return source;
  }
  const currentExport = "export{Yn as i,Jn as n,Gn as r,fr as t};";
  const patchedExport = "export{Yn as i,Jn as n,Gn as r,fr as t,chatgptLinuxReadAloudSettingsPage as ReadAloudSettings};";
  ...
  if (exportPattern.test(source)) {
    return source.replace(exportPattern, (_match, exports) =>
      exports.includes("chatgptLinuxReadAloudSettingsPage as ReadAloudSettings")
        ? _match
        : `export{${exports},chatgptLinuxReadAloudSettingsPage as ReadAloudSettings};`,
    );
  }
  return source;
}
...
function applyGeneralSettingsWrapperPatch(source) {
  ...
  const readAloudAlias = "chatgptLinuxReadAloudSettings";
...
function hasReadAloudSettingsIconDeclaration(source) {
  return /(?:^|[;\n])\s*(?:var|let|const)\s+chatgptLinuxReadAloudSettingsIcon(?:[=,;])/.test(source);
}

function declareLegacyReadAloudSettingsIconIfNeeded(source) {
  if (
    !source.includes("chatgptLinuxReadAloudSettingsIcon=e=>") ||
  ...
  return `${source.slice(0, insertionIndex)}var chatgptLinuxReadAloudSettingsIcon;${source.slice(insertionIndex)}`;
}
...
function applySettingsPageNavPatch(source) {
  ...
  const staleReadAloudIconRegex =
    /("computer-use":([A-Za-z_$][\w$]*),"read-aloud-settings":)(chatgptLinuxReadAloudSettingsIcon|[A-Za-z_$][\w$]*)(,"local-environments")/;
