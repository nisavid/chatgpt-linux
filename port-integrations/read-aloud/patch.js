...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function chatgptLinuxReadAloudKokoroPython(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_PYTHON?.trim(),t=chatgptLinuxReadAloudSettings()[${JSON.stringify(KOKORO_PYTHON_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(chatgptLinuxReadAloudDataHome(),\`chatgpt\`,\`read-aloud\`,\`kokoro-venv\`,\`bin\`,\`python\`)}`,
    `function chatgptLinuxReadAloudKokoroModel(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_MODEL?.trim(),t=chatgptLinuxReadAloudSettings()[${JSON.stringify(KOKORO_MODEL_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(chatgptLinuxReadAloudDataHome(),\`kokoro\`,\`kokoro-v1.0.onnx\`)}`,
    `function chatgptLinuxReadAloudKokoroVoices(){let e=process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICES?.trim(),t=chatgptLinuxReadAloudSettings()[${JSON.stringify(KOKORO_VOICES_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(chatgptLinuxReadAloudDataHome(),\`kokoro\`,\`voices-v1.0.bin\`)}`,
    ...
    `function chatgptLinuxReadAloudKokoroModelUrl(){return process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_MODEL_URL?.trim()||${JSON.stringify(KOKORO_MODEL_URL)}}`,
    `function chatgptLinuxReadAloudKokoroVoicesUrl(){return process.env.CHATGPT_LINUX_READ_ALOUD_KOKORO_VOICES_URL?.trim()||${JSON.stringify(KOKORO_VOICES_URL)}}`,
...
function readAloudRuntimeSource() {
  return [
    `;(()=>{const VERSION=${JSON.stringify(RUNTIME_VERSION)};if(globalThis.chatgptLinuxReadAloudVersion===VERSION)return;globalThis.chatgptLinuxReadAloudVersion=VERSION;try{globalThis.speechSynthesis?.cancel?.()}catch{}`,
...
function applyLinuxDesktopSettingsPatch(source) {
  ...
  if (!patched.includes(`readAloud:${JSON.stringify(SETTINGS_KEY)}`)) {
    ...
    patched = patched.replace(
      keyBoundary,
      `${keyNeedle},readAloud:${JSON.stringify(SETTINGS_KEY)},readAloudSpeed:${JSON.stringify(KOKORO_SPEED_KEY)}};function chatgptLinuxChecked`,
    );
  }
