...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxReadAloudSettingsAppId(){let e=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`codex-app\`;return/^[A-Za-z0-9._-]+$/.test(e)?e:\`codex-app\`}`,
    ...
    `function codexLinuxReadAloudEnabled(){if(process.env.CODEX_LINUX_READ_ALOUD_ENABLED===\`1\`)return!0;let e=codexLinuxReadAloudSettings();return e["codex-linux-read-aloud-enabled"]===!0}`,
    ...
    `function codexLinuxReadAloudKokoroPython(){let e=process.env.CODEX_LINUX_READ_ALOUD_KOKORO_PYTHON?.trim(),t=codexLinuxReadAloudSettings()[${JSON.stringify(KOKORO_PYTHON_KEY)}];return e||typeof t===\`string\`&&t.trim()||${pathVar}.join(codexLinuxReadAloudDataHome(),\`codex-app\`,\`read-aloud\`,\`kokoro-venv\`,\`bin\`,\`python\`)}`,
    ...
    `function codexLinuxReadAloudKokoroSpeed(){let e=process.env.CODEX_LINUX_READ_ALOUD_KOKORO_SPEED?.trim(),t=codexLinuxReadAloudSettings()["codex-linux-read-aloud-kokoro-speed"];return codexLinuxReadAloudClampSpeed(e!==void 0&&e!==\`\`?e:t??1.05)}`,
...
function applySettingsPatch(source) {
  return source
    .replace(`,readAloud:"codex-linux-read-aloud-enabled"`, "")
    .replace(
