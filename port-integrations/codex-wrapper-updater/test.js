...
function fakeManager(temp, body = "exit ${CODEX_FAKE_MANAGER_STATUS:-0}\n") {
  const manager = path.join(temp, "codex-app-updater");
...
test("settings patch adds wrapper update toggle", () => {
  ...
  assert.match(patched, /integrationPickerOnUpdate:"codex-linux-integration-picker-on-update"/);
  assert.match(patched, /Check for Codex App updates/);
...
test("integration exposes optional patches and declarative apply hooks when enabled", () => {
  withTempIntegrationConfig(["codex-wrapper-updater"], () => {
    ...
    assert.deepEqual(
      ...
      [
        ["integration:codex-wrapper-updater:main-handler", "main-bundle", "optional"],
        ["integration:codex-wrapper-updater:webview-runtime", "webview-asset", "optional"],
        ["integration:codex-wrapper-updater:settings-toggle", "extracted-app:post-webview", "optional"],
      ],
    );
