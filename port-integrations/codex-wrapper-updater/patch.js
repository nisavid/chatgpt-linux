...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function codexLinuxWrapAppId(){let i=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`codex-app\`;return /^[A-Za-z0-9._-]+$/.test(i)?i:\`codex-app\`}`,
    ...
    `function codexLinuxWrapStatePath(){let __codexWrapHome=codexLinuxWrapHome();let __codexWrapStateRoot=process.env.XDG_STATE_HOME||(__codexWrapHome&&codexLinuxWrapPath().join(__codexWrapHome,\`.local\`,\`state\`));return __codexWrapStateRoot?codexLinuxWrapPath().join(__codexWrapStateRoot,\`codex-app-updater\`,\`state.json\`):null}`,
    ...
    `function codexLinuxWrapStatusPayload(){let e=codexLinuxWrapWrapperUpdatesEnabled(),a=e&&codexLinuxWrapManagerAvailable(),s=a?codexLinuxWrapReadStatus():null;return{ok:!0,enabled:e,available:a,show:e&&a&&codexLinuxWrapShouldShow(s),dev_mode:!!(s&&s.wrapper_dev_mode===!0),changelog:s?s.wrapper_changelog||\`\`:\`\`,commit:s?s.candidate_wrapper_commit||\`\`:\`\`,installed_commit:s?s.installed_wrapper_commit||\`\`:\`\`}}`,
    ...
    `function codexLinuxWrapIsExecutable(p){try{codexLinuxWrapFs().accessSync(p,codexLinuxWrapFs().constants.X_OK);return!0}catch{return!1}}`,
    `function codexLinuxWrapFindOnPath(c){let p=process.env.PATH||\`\`;for(let d of p.split(\`:\`)){if(!d)continue;let f=codexLinuxWrapPath().join(d,c);if(codexLinuxWrapIsExecutable(f))return f}return null}`,
    ...
    `function codexLinuxWrapSpawnCheck(){if(!codexLinuxWrapWrapperUpdatesEnabled()||!codexLinuxWrapManagerAvailable())return;try{let __codexWrapCheckProcess=codexLinuxWrapChildProcess().spawn(codexLinuxWrapManagerPath(),[\`check-wrapper\`],{stdio:\`ignore\`,detached:!0,env:process.env});__codexWrapCheckProcess.on(\`error\`,()=>{});__codexWrapCheckProcess.unref()}catch{}}`,
    ...
    // `codex-linux-integration-picker-on-update` toggle (absent ⇒ ask) plus a live
    ...
    `function codexLinuxWrapSettingsAppId(){let e=process.env.CODEX_LINUX_APP_ID||process.env.CODEX_APP_ID||\`codex-app\`;return/^[A-Za-z0-9._-]+$/.test(e)?e:\`codex-app\`}`,
...
function applyWrapperUpdateSettingsPatch(source) {
  ...
  if (!next.includes("Check for Codex App updates")) {
    const toggleNeedle =
    ...
    const wrapperToggle =
      `children:[$.jsx(LinuxToggle,{settingKey:KEYS.autoUpdateOnExit,label:"Install updates when you close Codex",description:"When on, a ready update waits for Codex to close and then installs. When off, updates wait until you click Update."},"autoUpdateOnExit"),$.jsx(LinuxToggle,{settingKey:KEYS.wrapperUpdates,label:"Check for Codex App updates",description:"Check for Linux wrapper updates from codex-app in addition to upstream Codex app updates.",defaultValue:!1},"wrapperUpdates"),${pickerToggle}]`;
    ...
    const existingWrapperToggle =
      `$.jsx(LinuxToggle,{settingKey:KEYS.wrapperUpdates,label:"Check for Codex App updates",description:"Check for Linux wrapper updates from codex-app in addition to upstream Codex app updates.",defaultValue:!1},"wrapperUpdates")`;
