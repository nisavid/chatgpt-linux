...
const integrationId = "x11-ewmh-computer-use";
...
function portIntegrationsLib() {
  ...
  return require(path.join(repoRoot, "scripts/lib/port-integrations.js"));
}

function copyIntegrationTo(integrationsRoot) {
  const target = path.join(integrationsRoot, integrationId);
  fs.mkdirSync(target, { recursive: true });
  for (const file of ["integration.json", "README.md", "stage.sh", "patches.js"]) {
    fs.copyFileSync(path.join(integrationDir, file), path.join(target, file));
  }
...
function runStage(workspace, extraEnv = {}) {
  return execFileSync("bash", [path.join(integrationDir, "stage.sh")], {
    cwd: workspace,
    env: {
      ...process.env,
...
test("x11-ewmh-computer-use stays disabled until listed in integrations.json", () => {
  const { enabledPortIntegrationStageHooks, loadPortIntegrationPatchDescriptors } = portIntegrationsLib();
  const workspace = tempDir("x11-ewmh-integration");
  const integrationsRoot = path.join(workspace, "integrations");
  ...
  fs.writeFileSync(path.join(integrationsRoot, "integrations.json"), `{"enabled":["${integrationId}"]}\n`);
...
test("x11-ewmh-computer-use stage hook records marketplace entry and preserves computer-use", () => {
  ...
  execFileSync("bash", [path.join(integrationDir, "stage.sh")], {
    cwd: workspace,
    env: {
      ...process.env,
      SCRIPT_DIR: upstreamRepoRoot(),
