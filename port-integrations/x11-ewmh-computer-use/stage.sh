...
verify_sha256() {
    ...
    expected="$(expected_sha_value)" || {
        echo "CHATGPT_X11_COMPUTER_USE_RELEASE_SHA256 is required for tarball/download mode" >&2
        return 1
    }
    local actual
...
write_plugin_from_binary() {
    local binary="$1"
    local dest="$2"
    [ -x "$binary" ] || { echo "chatgpt-computer-use-x11 binary is not executable: $binary" >&2; return 1; }
    rm -rf "$dest"
    mkdir -p "$dest/.codex-plugin" "$dest/bin" "$dest/assets"
    cp "$binary" "$dest/bin/chatgpt-computer-use-x11"
    chmod 0755 "$dest/bin/chatgpt-computer-use-x11"
    ...
    cat > "$dest/.mcp.json" <<'JSON'
{
  "mcpServers": {
    "chatgpt-computer-use-x11": {
      "command": "./bin/chatgpt-computer-use-x11",
      "args": ["mcp"],
      "cwd": "."
    }
  }
...
stage_from_tarball() {
    ...
    chmod 0755 "$TARGET_PLUGIN/bin/chatgpt-computer-use-x11"
}

stage_from_source() {
    local source="$1"
    [ -f "$source/Cargo.toml" ] || { echo "CHATGPT_X11_COMPUTER_USE_SOURCE lacks Cargo.toml: $source" >&2; return 1; }
    local cargo_cmd
    cargo_cmd="$(find_cargo)" || { echo "cargo not found for CHATGPT_X11_COMPUTER_USE_SOURCE build" >&2; return 1; }
    ...
    write_plugin_from_binary "$cargo_target_dir/release/chatgpt-computer-use-x11" "$TARGET_PLUGIN"
}

stage_from_download() {
    local url="$1"
    local tarball="$WORK_DIR/chatgpt-computer-use-x11-download.tar.gz"
    if command -v curl >/dev/null 2>&1; then
        curl -fsSL "$url" -o "$tarball"
    elif command -v wget >/dev/null 2>&1; then
        wget -O "$tarball" "$url"
    else
        echo "curl or wget is required for CHATGPT_X11_COMPUTER_USE_DOWNLOAD_URL" >&2
        return 1
    fi
...
marketplace.plugins = marketplace.plugins.filter((plugin) => plugin?.name !== "chatgpt-computer-use-x11");
marketplace.plugins.push({
  name: "chatgpt-computer-use-x11",
  source: { source: "local", path: "./plugins/chatgpt-computer-use-x11" },
...
mkdir -p "$WORK_DIR"
if [ -n "${CHATGPT_X11_COMPUTER_USE_RELEASE_TARBALL:-}" ]; then
    stage_from_tarball "$CHATGPT_X11_COMPUTER_USE_RELEASE_TARBALL"
elif [ -n "${CHATGPT_X11_COMPUTER_USE_BINARY:-}" ]; then
    write_plugin_from_binary "$CHATGPT_X11_COMPUTER_USE_BINARY" "$TARGET_PLUGIN"
elif [ -n "${CHATGPT_X11_COMPUTER_USE_SOURCE:-}" ]; then
    stage_from_source "$CHATGPT_X11_COMPUTER_USE_SOURCE"
