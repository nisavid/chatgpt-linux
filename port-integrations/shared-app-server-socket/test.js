...
test("shared-app-server-socket is default-on but can be disabled", () => {
  ...
  withIntegrationConfig({ enabled: ["shared-app-server-socket"], disabled: [] }, (integrationsRoot) => {
    ...
    assert.deepEqual(
      ...
      ["integration:shared-app-server-socket:main-process-shared-app-server-socket"],
    );
  });
});
...
test("integration stages only its socket environment hook", () => {
  withIntegrationConfig({ enabled: ["shared-app-server-socket"], disabled: [] }, (integrationsRoot) => {
    ...
    try {
      ...
      const runtimeHooks = fullPlan.runtimeHooks.filter((hook) => hook.id === "shared-app-server-socket");
      assert.deepEqual(
        runtimeHooks.map((hook) => [hook.key, path.basename(hook.target), hook.mode.toString(8)]),
...
test("socket hook is inert until runtime opt-in", () => {
