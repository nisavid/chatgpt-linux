...
When enabled, the integration stages the bundled `agent-workspace-linux` skill
...
the agent-facing progressive routing entrypoint; the integration does not write
...
matching this integration's app-owned configuration model. If a previous v0.1.0
...
workspace start/stop, and viewer launch. The integration does not require the user
...
unchanged by this integration. Agent-facing workspace tools are expected to be
...
The integration does not maintain its own hand-listed table of per-action approval
...
The integration intentionally does not inject a conversation workspace screen. The
...
The integration only patches the Electron bridge and the Settings webview bundles;
...
```bash
node --test port-integrations/agent-workspace/test.js
```
