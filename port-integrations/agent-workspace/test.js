...
test("agent-workspace integration exposes optional bridge, settings, resources, and prelaunch hook when enabled", () => {
  withTempIntegrationConfig(["agent-workspace"], (root) => {
    assert.deepEqual(enabledPortIntegrationIds({ integrationsRoot: root }), ["agent-workspace"]);
    assert.deepEqual(
    ...
    const plan = enabledPortIntegrationInstallPlan({ integrationsRoot: root });
    assert.deepEqual(
...
test("agent-workspace prelaunch hook installs the staged bundled Codex skill only", () => {
  ...
  const hook = path.join(integrationDir, "install-skill.sh");
  const skillSource = path.join(integrationDir, "skills", "agent-workspace-linux", "SKILL.md");
  ...
  try {
    ...
    const result = spawnSync("bash", [hook], {
      cwd: path.resolve(integrationDir, "../.."),
      encoding: "utf8",
      env: {
        ...process.env,
  ...
  await withTempIntegrationConfigAsync(["agent-workspace"], async (integrationsRoot) => {
    await withPortIntegrationRootEnvAsync(integrationsRoot, async () => {
      ...
      try {
        ...
        assert.ok(report.patches.some((patch) => patch.name === "integration:agent-workspace:main-bridge" && patch.status === "applied"));
        assert.ok(report.patches.some((patch) => patch.name === "integration:agent-workspace:settings-page" && patch.status === "applied"));
        assert.equal(report.patches.some((patch) => patch.name === "integration:agent-workspace:conversation-view"), false);
        assert.equal(report.patches.some((patch) => patch.name === "integration:agent-workspace:stale-runtime-cleanup"), false);
        assert.equal(report.patches.some((patch) => patch.name === "integration:agent-workspace:approval-rendering"), false);
      } finally {
