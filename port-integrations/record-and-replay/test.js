...
test("manifest keeps record-and-replay disabled by default", () => {
  const manifestPath = path.join(__dirname, "integration.json");
...
test("record-and-replay required files exist", () => {
  assert.equal(fs.existsSync(path.join(__dirname, "integration.json")), true);
...
test("record-and-replay is opt-in and disabled unless configured", () => {
  withTempFeatureRoot([], (root) => {
    ...
    assert.deepEqual(loadEnabledPortIntegrations({ integrationsRoot: root }), []);
  });
});

test("record-and-replay enables when listed in integrations.json", () => {
  withTempFeatureRoot(["record-and-replay"], (root) => {
    const ids = enabledPortIntegrationIds({ integrationsRoot: root });
    assert.deepEqual(ids, ["record-and-replay"]);
    assert.deepEqual(loadEnabledPortIntegrations({ integrationsRoot: root }).map((feature) => feature.id), [
      "record-and-replay",
    ]);
  });
});
...
test("record-and-replay patch descriptor loads only when feature is enabled", () => {
  ...
  withTempFeatureConfig(["record-and-replay"], (root) => {
    const loaded = loadPortIntegrationPatchDescriptors({ integrationsRoot: root });
    assert.deepEqual(loaded.map((descriptor) => descriptor.id), [
      "integration:record-and-replay:record-and-replay-plugin-gate",
      "integration:record-and-replay:linux-record-replay-main-bridge",
      "integration:record-and-replay:record-replay-hud",
      "integration:record-and-replay:record-replay-dictation-transcript",
      "integration:record-and-replay:record-replay-global-dictation-transcript",
    ]);
...
test("record-and-replay disabled rebuild exposes cleanup hook for staged payload", () => {
  ...
  try {
    ...
    stageEnabledPortIntegrationInstall(installDir, { integrationsRoot });
