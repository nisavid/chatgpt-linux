...
function recordReplayBridgeSource({ childProcessVar, fsVar, pathVar }) {
  return [
    `"chronicle-permissions":async()=>{let e=await chatgptLinuxChronicleSidecarControlStateAsync(),t=e.enabled===!0?"granted":"unknown";return{accessibility:t,screenRecording:t,chronicleSidecarPresent:e.enabled===!0,chronicleSidecarProcessState:e.state??"disabled",chronicleOcrAvailable:e.chronicleOcrAvailable===!0,chronicleOcrStatus:e.chronicleOcrStatus??"unknown",chronicleOcrBackend:e.chronicleOcrBackend??null,chronicleOcrLanguage:e.chronicleOcrLanguage??null}}`,
    `"getChronicleSidecarControlState":async()=>chatgptLinuxChronicleSidecarControlStateAsync()`,
    `"toggleChronicleSidecar":async()=>chatgptLinuxChronicleToggleSidecar()`,
    `"linux-record-replay-doctor":async()=>chatgptLinuxRecordReplayRun([${JSON.stringify("doctor")}],15000)`,
    `"linux-record-replay-status":async()=>chatgptLinuxRecordReplayRun([${JSON.stringify("status")}],5000)`,
    `"linux-record-replay-start":async({sessionDir:e,appId:t,windowId:n,goal:r,includeScreenshot:a,includeAccessibility:o,includeAudio:s}={})=>{let i=chatgptLinuxRecordReplayString(e);if(!i)return{ok:!1,action:"record.start",message:"sessionDir is required"};let c=["record","start","--session-dir",i];t&&c.push("--app-id",String(t));n&&c.push("--window-id",String(n));r&&c.push("--goal",String(r));a===!1&&c.push("--no-screenshot");o===!1&&c.push("--no-accessibility");s===!0&&c.push("--audio");s===!1&&c.push("--no-audio");return chatgptLinuxRecordReplayRun(c,60000)}`,
    `"linux-record-replay-skysight-start":async({intervalSeconds:e,summaryAgent:t}={})=>{let n=["skysight","start"];e&&n.push("--interval-seconds",String(e));t===!0&&n.push("--summary-agent","enabled");t===!1&&n.push("--summary-agent","disabled");return chatgptLinuxRecordReplayRun(n,15000)}`,
    `"linux-record-replay-skysight-status":async()=>chatgptLinuxRecordReplayRun(["skysight","status"],5000)`,
    `"linux-record-replay-skysight-pause":async()=>chatgptLinuxRecordReplayRun(["skysight","pause"],10000)`,
    `"linux-record-replay-skysight-resume":async()=>chatgptLinuxRecordReplayRun(["skysight","resume"],10000)`,
    `"linux-record-replay-skysight-stop":async()=>chatgptLinuxRecordReplayRun(["skysight","stop"],10000)`,
    `"linux-record-replay-skysight-snapshot":async({source:e}={})=>{let t=["skysight","snapshot"];e&&t.push("--source",String(e));return chatgptLinuxRecordReplayRun(t,15000)}`,
    `"linux-record-replay-skysight-list-exclusions":async()=>chatgptLinuxRecordReplayRun(["skysight","list-exclusions"],5000)`,
    `"linux-record-replay-skysight-update-exclusion":async({kind:e,value:t,reason:n,remove:r}={})=>{let a=chatgptLinuxRecordReplayString(e),o=chatgptLinuxRecordReplayString(t);if(!a||!o)return{ok:!1,action:"skysight.update-exclusion",message:"kind and value are required"};let s=["skysight","update-exclusion","--kind",a,"--value",o];n&&s.push("--reason",String(n));r&&s.push("--remove");return chatgptLinuxRecordReplayRun(s,10000)}`,
    `"linux-record-replay-mark":async({sessionDir:e,note:t}={})=>{let n=chatgptLinuxRecordReplayString(e),r=chatgptLinuxRecordReplayString(t);if(!n||!r)return{ok:!1,action:"record.mark",message:"sessionDir and note are required"};return chatgptLinuxRecordReplayRun(["record","mark","--session-dir",n,"--note",r],15000)}`,
    `"linux-record-replay-speech-context":async({sessionDir:e,transcript:t,source:n}={})=>{let r=chatgptLinuxRecordReplayString(e),a=chatgptLinuxRecordReplayString(t);if(!r||!a)return{ok:!1,action:"record.speech",message:"sessionDir and transcript are required"};let o=["record","speech","--session-dir",r,"--text",a];n&&o.push("--source",String(n));return chatgptLinuxRecordReplayRun(o,15000)}`,
    recordReplayActiveSpeechContextBridgeHandler(),
    `"linux-record-replay-browser-trace":async({sessionDir:e,trace:t,traceJson:n,url:r,title:a,source:o}={})=>{let s=chatgptLinuxRecordReplayString(e),i=chatgptLinuxRecordReplayString(n);if(!s)return{ok:!1,action:"record.browser-trace",message:"sessionDir is required"};if(!i&&t!==void 0)i=JSON.stringify(t);if(!i)return{ok:!1,action:"record.browser-trace",message:"trace or traceJson is required"};let c=chatgptLinuxRecordReplayWriteTempJson(i),l=["record","browser-trace","--session-dir",s,"--trace-file",c];r&&l.push("--url",String(r));a&&l.push("--title",String(a));o&&l.push("--source",String(o));try{return await chatgptLinuxRecordReplayRun(l,30000)}finally{try{${fsVar}.unlinkSync(c)}catch{}}}`,
    `"linux-record-replay-desktop-snapshot":async({sessionDir:e,source:t}={})=>{let n=chatgptLinuxRecordReplayString(e);if(!n)return{ok:!1,action:"record.desktop-snapshot",message:"sessionDir is required"};let r=["record","desktop-snapshot","--session-dir",n];t&&r.push("--source",String(t));return chatgptLinuxRecordReplayRun(r,15000)}`,
    `"linux-record-replay-stop":async({sessionDir:e}={})=>{let t=chatgptLinuxRecordReplayString(e);if(!t)return{ok:!1,action:"record.stop",message:"sessionDir is required"};return chatgptLinuxRecordReplayRun(["record","stop","--session-dir",t],15000)}`,
    `"linux-record-replay-stop-active":async()=>{let e=await chatgptLinuxRecordReplayRun(["status"],5000),t=e?.json?.session_dir;if(!e?.ok||e?.json?.state!==\`active\`||!t)return{ok:!1,action:"record.stop-active",message:"No active Record & Replay session"};return chatgptLinuxRecordReplayRun(["record","stop","--session-dir",String(t)],15000)}`,
    `"linux-record-replay-cancel":async({sessionDir:e,discarded:t}={})=>{let n=chatgptLinuxRecordReplayString(e);if(!n)return{ok:!1,action:"record.cancel",message:"sessionDir is required"};let r=["record","cancel","--session-dir",n];t&&r.push("--discarded");return chatgptLinuxRecordReplayRun(r,15000)}`,
    `"linux-record-replay-cancel-active":async({discarded:e}={})=>{let t=await chatgptLinuxRecordReplayRun(["status"],5000),n=t?.json?.session_dir;if(!t?.ok||t?.json?.state!==\`active\`||!n)return{ok:!1,action:"record.cancel-active",message:"No active Record & Replay session"};let r=["record","cancel","--session-dir",String(n)];e&&r.push("--discarded");return chatgptLinuxRecordReplayRun(r,15000)}`,
    `"linux-record-replay-bundle":async({bundle:e}={})=>{let t=chatgptLinuxRecordReplayString(e);if(!t)return{ok:!1,action:"bundle.validate",message:"bundle is required"};return chatgptLinuxRecordReplayRun(["bundle","validate","--bundle",t],15000)}`,
    `"linux-record-replay-draft-skill":async({bundle:e}={})=>{let t=chatgptLinuxRecordReplayString(e);if(!t)return{ok:!1,action:"bundle.draft-prompt",message:"bundle is required"};return chatgptLinuxRecordReplayRun(["bundle","draft-prompt","--bundle",t],30000)}`,
    `"linux-record-replay-import-skill":async({source:e,dryRun:t,allowUnsupported:n}={})=>{let r=chatgptLinuxRecordReplayString(e);if(!r)return{ok:!1,action:"skill.import",message:"source is required"};let a=["skill","import","--source",r];t&&a.push("--dry-run");n&&a.push("--allow-unsupported");return chatgptLinuxRecordReplayRun(a,30000)}`,
    `"linux-record-replay-inspect-skill":async({source:e}={})=>{let t=chatgptLinuxRecordReplayString(e);if(!t)return{ok:!1,action:"skill.inspect",message:"source is required"};return chatgptLinuxRecordReplayRun(["skill","inspect","--source",t],15000)}`,
  ].join(",");
}

function recordReplayActiveSpeechContextBridgeHandler() {
  return `"linux-record-replay-speech-context-active":async({transcript:e,source:t}={})=>{let n=chatgptLinuxRecordReplayString(e);if(!n)return{ok:!1,action:"record.speech-active",message:"transcript is required"};let r=await chatgptLinuxRecordReplayRun(["status"],5000),a=r?.json?.session_dir;if(!r?.ok||r?.json?.state!==\`active\`||!a)return{ok:!1,action:"record.speech-active",message:"No active Record & Replay session",status:r};let o=["record","speech","--session-dir",String(a),"--text",n];t&&o.push("--source",String(t));return chatgptLinuxRecordReplayRun(o,15000)}`;
}

function recordReplayHelperSource({ childProcessVar, fsVar, pathVar }) {
  return `function chatgptLinuxRecordReplayString(e){return typeof e==="string"&&e.trim().length>0?e.trim():null}
function chatgptLinuxRecordReplayBin(){let e=chatgptLinuxRecordReplayString(process.env.CHATGPT_RECORD_REPLAY_LINUX_BIN);if(e)return e;let t=[];try{process.resourcesPath&&t.push(${pathVar}.join(process.resourcesPath,"native","chatgpt-record-replay-linux"))}catch{}try{t.push(${pathVar}.join(process.cwd(),"resources","native","chatgpt-record-replay-linux"))}catch{}try{let e=process.env.PATH||"";for(let n of e.split(${pathVar}.delimiter))n&&t.push(${pathVar}.join(n,"chatgpt-record-replay-linux"))}catch{}t.push("chatgpt-record-replay-linux");for(let e of t){try{if(e==="chatgpt-record-replay-linux"||${fsVar}.existsSync(e))return e}catch{}}return "chatgpt-record-replay-linux"}
function chatgptLinuxRecordReplayParse(e){let t=String(e||"").trim();if(!t)return null;try{return JSON.parse(t)}catch{return{raw:t}}}
function chatgptLinuxRecordReplayWriteTempJson(e){let t=process.env.XDG_RUNTIME_DIR||process.env.TMPDIR||"/tmp",n=${pathVar}.join(t,"chatgpt-record-replay-traces-"+(process.getuid?process.getuid():process.pid));${fsVar}.mkdirSync(n,{recursive:true,mode:448});try{if(${fsVar}.lstatSync(n).isSymbolicLink())throw Error("trace directory is a symlink");${fsVar}.chmodSync(n,448)}catch(r){throw r}let r=${pathVar}.join(n,"trace-"+Date.now()+"-"+process.pid+"-"+Math.random().toString(36).slice(2)+".json");${fsVar}.writeFileSync(r,String(e),{mode:384});return r}
function chatgptLinuxRecordReplayRun(e,t){let n=chatgptLinuxRecordReplayBin();return new Promise(r=>{${childProcessVar}.execFile(n,e,{encoding:"utf8",timeout:t,maxBuffer:16777216},(t,a,o)=>{let s=chatgptLinuxRecordReplayParse(a);if(t)return r({ok:!1,command:n,args:e,message:t instanceof Error?t.message:String(t),code:t?.code??null,stdout:a||"",stderr:o||"",json:s});r({ok:!0,command:n,args:e,stdout:a||"",stderr:o||"",json:s})})})}
${recordReplayChronicleHelperSource({ childProcessVar })}`;
}

function recordReplayChronicleHelperSource({ childProcessVar }) {
  return `function chatgptLinuxRecordReplayRunSync(e,t){let n=chatgptLinuxRecordReplayBin();try{let r=${childProcessVar}.execFileSync(n,e,{encoding:"utf8",timeout:t,maxBuffer:16777216});return{ok:!0,command:n,args:e,stdout:r||"",stderr:"",json:chatgptLinuxRecordReplayParse(r)}}catch(r){let a=typeof r?.stdout==="string"?r.stdout:r?.stdout?String(r.stdout):"",o=typeof r?.stderr==="string"?r.stderr:r?.stderr?String(r.stderr):"";return{ok:!1,command:n,args:e,message:r instanceof Error?r.message:String(r),code:r?.status??r?.code??null,stdout:a,stderr:o,json:chatgptLinuxRecordReplayParse(a)}}}
function chatgptLinuxChronicleControlStateFromSkysight(e){let t=e?.json&&typeof e.json==="object"?e.json:null;if(!e?.ok&&t==null)return{enabled:!1,running:!1,state:"disabled"};let n=String(t?.state||""),r=t?.is_running===!0||t?.isRunning===!0,a=t?.paused===!0||t?.is_paused===!0||t?.isPaused===!0||n==="paused",o=n==="running"&&r&&!a;return{enabled:!0,running:o,state:o?"running":"stopped",skysight:t,chronicleOcrAvailable:t?.ocr_available===!0||t?.ocrAvailable===!0,chronicleOcrStatus:t?.ocr_status??t?.ocrStatus??"unknown",chronicleOcrBackend:t?.ocr_backend??t?.ocrBackend??null,chronicleOcrLanguage:t?.ocr_language??t?.ocrLanguage??null}}
function chatgptLinuxChronicleSidecarControlState(){return chatgptLinuxChronicleControlStateFromSkysight(chatgptLinuxRecordReplayRunSync(["skysight","status"],3000))}
async function chatgptLinuxChronicleSidecarControlStateAsync(){return chatgptLinuxChronicleControlStateFromSkysight(await chatgptLinuxRecordReplayRun(["skysight","status"],5000))}
function chatgptLinuxChronicleSummaryAgentArgs(e){return e===!0?["--summary-agent","enabled"]:e===!1?["--summary-agent","disabled"]:[]}
async function chatgptLinuxChronicleEnsureSidecarRunning(e){let t=await chatgptLinuxRecordReplayRun(["skysight","status"],5000),n=t?.json&&typeof t.json==="object"?t.json:null,r=String(n?.state||""),a=n?.is_running===!0||n?.isRunning===!0,o=n?.paused===!0||n?.is_paused===!0||n?.isPaused===!0||r==="paused",s=chatgptLinuxChronicleSummaryAgentArgs(e),i=e===!0&&(n?.summary_agent_enabled!==!0&&n?.summaryAgentEnabled!==!0);if(r==="running"&&a&&!o)return i?chatgptLinuxChronicleControlStateFromSkysight(await chatgptLinuxRecordReplayRun(["skysight","start",...s],15000)):chatgptLinuxChronicleControlStateFromSkysight(t);if(a&&o){i&&await chatgptLinuxRecordReplayRun(["skysight","start",...s],15000);return chatgptLinuxChronicleControlStateFromSkysight(await chatgptLinuxRecordReplayRun(["skysight","resume"],10000))}return chatgptLinuxChronicleControlStateFromSkysight(await chatgptLinuxRecordReplayRun(["skysight","start",...s],15000))}
async function chatgptLinuxChronicleToggleSidecar(){let e=await chatgptLinuxRecordReplayRun(["skysight","status"],5000),t=e?.json&&typeof e.json==="object"?e.json:null,n=String(t?.state||""),r=t?.is_running===!0||t?.isRunning===!0,a=t?.paused===!0||t?.is_paused===!0||t?.isPaused===!0||n==="paused";if(n==="running"&&r&&!a)return chatgptLinuxChronicleControlStateFromSkysight(await chatgptLinuxRecordReplayRun(["skysight","pause"],10000));if(r&&a)return chatgptLinuxChronicleEnsureSidecarRunning(!0);return chatgptLinuxChronicleControlStateFromSkysight(await chatgptLinuxRecordReplayRun(["skysight","start","--summary-agent","enabled"],15000))}`;
}
...
function recordReplayChronicleTrayPatchedPattern() {
  return /getChronicleSidecarControlState:\(\)=>process\.platform===`linux`\?chatgptLinuxChronicleSidecarControlState\(\):([A-Za-z_$][\w$]*)\(\)\.skysight\?([A-Za-z_$][\w$]*):([A-Za-z_$][\w$]*\.appServerConnectionRegistry\.getMaybeConnection\(`local`\)\?\.getChronicleSidecarControlState\(\)\?\?\2),toggleChronicleSidecar:async\(\)=>\{if\(process\.platform===`linux`\)return chatgptLinuxChronicleToggleSidecar\(\);if\(\1\(\)\.skysight\)return \2;let ([A-Za-z_$][\w$]*)=([A-Za-z_$][\w$]*\.appServerConnectionRegistry\.getMaybeConnection\([A-Za-z_$][\w$]*\));return \4==null\?\2:\4\.getChronicleSidecarControlState\(\)\.running\?\4\.pauseChronicleSidecar\(\):\4\.resumeChronicleSidecar\(\)\}/u;
}
...
function hasAnyRecordReplayMainBridgeArtifact(source) {
  return source.includes("chatgptLinuxRecordReplay")
    || source.includes("chatgptLinuxChronicle")
...
function applyRecordReplayChronicleTrayPatch(currentSource) {
  ...
  if (currentSource.includes("process.platform===`linux`?chatgptLinuxChronicleSidecarControlState()")) {
    return currentSource;
  }
...
function recordReplayHudRuntimeSource() {
  return [
    ...
    `if(globalThis.chatgptLinuxRecordReplayHudVersion===VERSION)return;`,
    `globalThis.chatgptLinuxRecordReplayHudVersion=VERSION;`,
    ...
    `function post(method,params,timeoutMs=4000){let requestId="chatgpt-linux-record-replay-"+ ++seq;let payload={type:"fetch",hostId:"local",requestId,method:"POST",url:"vscode://codex/"+method,body:JSON.stringify(params??{})};return new Promise((resolve,reject)=>{let timer=setTimeout(()=>{pending.delete(requestId);reject(Error("timeout"))},timeoutMs);pending.set(requestId,{resolve,reject,timer});dispatch(payload)})}`,
    `function installStyle(){if(document.getElementById("chatgpt-linux-record-replay-hud-style"))return;let s=document.createElement("style");s.id="chatgpt-linux-record-replay-hud-style";s.textContent=".chatgpt-linux-record-replay-hud{position:fixed;left:50%;bottom:96px;transform:translateX(-50%);z-index:2147483002;height:42px;min-width:166px;display:none;align-items:center;gap:10px;padding:0 8px 0 12px;border-radius:14px;background:rgba(49,49,52,.94);border:1px solid rgba(255,255,255,.08);box-shadow:0 14px 34px rgba(0,0,0,.26);color:#f4f4f5;font:600 12px/1 ui-sans-serif,-apple-system,BlinkMacSystemFont,\\"Segoe UI\\",sans-serif;pointer-events:auto;-webkit-app-region:no-drag;backdrop-filter:blur(16px)}.chatgpt-linux-record-replay-hud[data-active=\\"true\\"]{display:flex}.chatgpt-linux-record-replay-dot{width:14px;height:14px;border-radius:999px;border:2px solid rgba(239,82,72,.62);position:relative;box-sizing:border-box}.chatgpt-linux-record-replay-dot::after{content:\\"\\";position:absolute;inset:2px;border-radius:999px;background:#ef4444}.chatgpt-linux-record-replay-time{min-width:29px;color:#ef4444;font-variant-numeric:tabular-nums}.chatgpt-linux-record-replay-btn{width:28px;height:30px;display:inline-flex;align-items:center;justify-content:center;border:0;border-left:1px solid rgba(255,255,255,.08);border-radius:0;background:transparent;color:#d4d4d8;cursor:pointer}.chatgpt-linux-record-replay-btn:hover{color:#fff;background:rgba(255,255,255,.06);border-radius:8px}.chatgpt-linux-record-replay-btn:disabled{opacity:.6;cursor:default}.chatgpt-linux-record-replay-btn svg{width:16px;height:16px}.chatgpt-linux-record-replay-grip{width:18px;height:30px;display:inline-flex;align-items:center;justify-content:center;border-left:1px solid rgba(255,255,255,.08);color:#a1a1aa}.chatgpt-linux-record-replay-grip::before{content:\\"\\";width:3px;height:18px;border-radius:999px;background:linear-gradient(to bottom,#a1a1aa 0 2px,transparent 2px 5px,#a1a1aa 5px 7px,transparent 7px 10px,#a1a1aa 10px 12px,transparent 12px 15px,#a1a1aa 15px 17px)}";document.head?.appendChild?.(s)}`,
    `function ensureHud(){if(hud&&document.contains(hud))return hud;installStyle();let root=document.createElement("div");root.id="chatgpt-linux-record-replay-hud";root.className="chatgpt-linux-record-replay-hud";root.setAttribute("role","status");root.setAttribute("aria-live","polite");root.innerHTML='<span class="chatgpt-linux-record-replay-dot" aria-hidden="true"></span><span class="chatgpt-linux-record-replay-time">0:00</span><button type="button" class="chatgpt-linux-record-replay-btn chatgpt-linux-record-replay-finish" title="Finish recording" aria-label="Finish Record & Replay recording"><svg aria-hidden="true" viewBox="0 0 24 24" fill="currentColor"><rect x="7" y="7" width="10" height="10" rx="1"></rect></svg></button><button type="button" class="chatgpt-linux-record-replay-btn chatgpt-linux-record-replay-discard" title="Discard recording" aria-label="Discard Record & Replay recording"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 6h18"></path><path d="M8 6V4h8v2"></path><path d="M10 11v6"></path><path d="M14 11v6"></path><path d="M5 6l1 15h12l1-15"></path></svg></button><span class="chatgpt-linux-record-replay-grip" aria-hidden="true"></span>';root.querySelector(".chatgpt-linux-record-replay-finish")?.addEventListener("click",finishRecording);root.querySelector(".chatgpt-linux-record-replay-discard")?.addEventListener("click",discardRecording);(document.body||document.documentElement).appendChild(root);hud=root;return root}`,
    ...
    `function drainBootstrapTranscriptQueue(){let q=globalThis.chatgptLinuxRecordReplayPendingTranscripts;if(!Array.isArray(q)||q.length===0)return;globalThis.chatgptLinuxRecordReplayPendingTranscripts=[];for(let entry of q){let transcript=typeof entry==="string"?entry:entry?.transcript,action=typeof entry==="object"?entry.action:"unknown",queuedAt=typeof entry==="object"&&Number.isFinite(entry.queuedAt)?entry.queuedAt:Date.now(),item=normalizeTranscript(transcript,action);item&&(item.queuedAt=queuedAt,rememberPendingTranscript(item))}}`,
    `async function writeTranscript(item){let transcript=item.transcript,action=item.action,status=active(lastStatus)?lastStatus:null;if(!status){try{let body=await post("linux-record-replay-status",{},2500);status=statusFrom(body);lastStatus=status;updateHud()}catch{return false}}if(!active(status))return false;let sessionDir=sessionDirFrom(status);if(!sessionDir)return false;let key=sessionDir+"|"+action+"|"+transcript;if(capturedTranscripts.has(key))return true;rememberTranscriptKey(key);try{await post("linux-record-replay-speech-context",{sessionDir,transcript,source:"chatgpt-dictation-"+action},15000);return true}catch{capturedTranscripts.delete(key);return false}}`,
    ...
    `function updateHud(){let h=ensureHud(),on=active(lastStatus);h.dataset.active=on?"true":"false";if(!on)return;let time=h.querySelector(".chatgpt-linux-record-replay-time");time&&(time.textContent=elapsedText(lastStatus));h.title=lastStatus?.goal?("Recording: "+lastStatus.goal):"Record & Replay recording active";positionHud()}`,
    ...
    `globalThis.chatgptLinuxRecordReplayCaptureTranscript=captureTranscript;`,
    `start();`,
    `})();`,
  ].join("");
}
...
function recordReplayTranscriptCaptureExpression(transcriptVar, actionVar) {
  return `(globalThis.chatgptLinuxRecordReplayCaptureTranscript?.(${transcriptVar},${actionVar})??((globalThis.chatgptLinuxRecordReplayPendingTranscripts??=[]).push({transcript:${transcriptVar},action:${actionVar},queuedAt:Date.now()}),!1))`;
}

function recordReplayActiveSpeechContextExpression(dispatchVar, transcriptVar) {
  return `(()=>{let t=String(${transcriptVar}??"").trim();if(t.length>0){let n="chatgpt-linux-record-replay-global-dictation-"+Date.now()+"-"+Math.random().toString(36).slice(2);${dispatchVar}.dispatchMessage("fetch",{hostId:"local",requestId:n,method:"POST",url:"vscode://codex/linux-record-replay-speech-context-active",body:JSON.stringify({transcript:t,source:"chatgpt-global-dictation"})})}})()`;
}

function recordReplayConversationTranscriptPattern() {
  return /([A-Za-z_$][\w$]*)\.length>0&&([A-Za-z_$][\w$]*)!==`discard`&&globalThis\.chatgptLinuxConversationShouldSendTranscript\?\.\(\1,\2\)!==!1&&\((([A-Za-z_$][\w$]*)\.getInstance\(\)\.dispatchMessage\(`global-dictation-record-history-item`,\{text:\1\}\),\2===`send`\?([A-Za-z_$][\w$]*)\.onTranscriptSend\(\1\):\5\.onTranscriptInsert\(\1\))\)/u;
}
...
function applyRecordReplayHudPatch(currentSource) {
  if (currentSource.includes("chatgptLinuxRecordReplayHudVersion=")) {
    return currentSource;
  }
...
function applyRecordReplayDictationTranscriptPatch(currentSource) {
  ...
  if (currentSource.includes("chatgptLinuxRecordReplayCaptureTranscript")) {
    return currentSource;
  }
  ...
  if (conversationPattern.test(currentSource)) {
    return currentSource.replace(
      ...
      (_, transcriptVar, actionVar, dispatchExpression) =>
        `${transcriptVar}.length>0&&${actionVar}!==\`discard\`&&globalThis.chatgptLinuxConversationShouldSendTranscript?.(${transcriptVar},${actionVar})!==!1&&(${recordReplayTranscriptCaptureExpression(transcriptVar, actionVar)},${dispatchExpression})`,
    );
  }
...
function hasRecordReplayDictationTranscriptContract(source) {
  ...
  if (source.includes("chatgptLinuxRecordReplayCaptureTranscript")) {
    return true;
  }
...
function applyRecordReplayGlobalDictationTranscriptPatch(currentSource) {
  ...
  if (currentSource.includes("chatgpt-linux-record-replay-global-dictation")) {
    return currentSource;
  }
