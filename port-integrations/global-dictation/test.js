...
function mainBundleFixture() {
  return [
    ...
    "function eA(e,t,n){let r=n?.ownership,i=t.onReleased,a=r==null?t:{onPressed:()=>{r.isOwner()&&t.onPressed()},onReleased:i==null?void 0:()=>{r.isOwner()&&i()}};if(Rk(e))return Lk(e)?Mk(e,a,n?.bareModifierTrigger):null;let o=oA(e),s=()=>{a.onPressed()},d=c.globalShortcut.register(o,s);return d?process.platform===`darwin`?sA({hotkey:e,onPressed:s,registrationHotkey:o}):{handlesRelease:!1,unregister:()=>{c.globalShortcut.unregister(o)}}:null}",
...
test("global dictation stays disabled until selected", () => {
  ...
  try {
    ...
    fs.writeFileSync(configPath, `${JSON.stringify(integrationSelection(integrationsRoot, ["global-dictation"]))}\n`);
    const loaded = loadPortIntegrationPatchDescriptors({ integrationsRoot });
    assert.deepEqual(loaded.map((descriptor) => descriptor.id), [
      "integration:global-dictation:linux-global-dictation-main-process",
    ]);
...
test("Wayland registration uses a release-aware portal helper", () => {
  ...
  assert.match(
    patched,
    /chatgptLinuxGlobalDictationPortalRegistration\(e,\{\.\.\.a,onUnavailable:t\.onUnavailable\}\)/,
  );
});
...
test("portal failure tears down the existing lifecycle without dropping other platforms", () => {
  ...
  assert.match(patched, /onUnavailable:n=>\{this\.handleLinuxHotkeyUnavailable\(`hold`,n\)\}\},\{ownership:t\}/);
  assert.match(patched, /onUnavailable:n=>\{this\.handleLinuxHotkeyUnavailable\(`toggle`,n\)\}\},\{bareModifierTrigger:`release`,ownership:t\}/);
});
...
test("main patch reports a missing release watcher sentinel as an optional skip", () => {
  ...
  const [descriptor] = normalizePatchDescriptors([
    {
      ...descriptors[0],
      id: "integration:global-dictation:linux-global-dictation-main-process",
      sourceKind: "integration",
      integrationId: "global-dictation",
    },
  ]);
  ...
  assert.deepEqual(report.patches, [
    {
      name: "integration:global-dictation:linux-global-dictation-main-process",
