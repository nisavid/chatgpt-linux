...
test("authenticated-proxy stays disabled until listed in integrations.json", () => {
  withFeatureConfig([], (integrationsRoot) => {
    assert.deepEqual(loadPortIntegrationPatchDescriptors({ integrationsRoot }), []);
  });

  withFeatureConfig(["authenticated-proxy"], (integrationsRoot) => {
    ...
    assert.deepEqual(
      ...
      [["integration:authenticated-proxy:main-process-proxy-auth", "main-bundle", "optional"]],
    );
  });
});
