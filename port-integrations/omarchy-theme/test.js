...
test("omarchy-theme is default-on but can be disabled", () => {
  withIntegrationConfig({ disabled: ["omarchy-theme"] }, (integrationsRoot) => {
    assert.equal(enabledPortIntegrationIds({ integrationsRoot }).includes("omarchy-theme"), false);
    assert.equal(loadPortIntegrationPatchDescriptors({ integrationsRoot }).some((entry) => entry.id.startsWith("integration:omarchy-theme:")), false);
  });
});

test("omarchy-theme exposes optional patches, resources, and hooks when enabled", () => {
  withIntegrationConfig({ enabled: ["omarchy-theme"] }, (integrationsRoot) => {
    assert.equal(enabledPortIntegrationIds({ integrationsRoot }).includes("omarchy-theme"), true);

    const descriptors = loadPortIntegrationPatchDescriptors({ integrationsRoot }).filter((entry) => entry.id.startsWith("integration:omarchy-theme:"));
    assert.equal(descriptors.length, 1);
    assert.equal(descriptors[0].id, "integration:omarchy-theme:omarchy-theme-css-loader");
    assert.equal(descriptors[0].ciPolicy, "optional");
    assert.match("index-main.js", descriptors[0].pattern);

    const fullPlan = enabledPortIntegrationInstallPlan({ integrationsRoot });
    const plan = {
      resources: fullPlan.resources.filter((entry) => entry.id === "omarchy-theme"),
      runtimeHooks: fullPlan.runtimeHooks.filter((entry) => entry.id === "omarchy-theme"),
    };
    assert.deepEqual(
...
test("prelaunch hook is a strict no-op when Omarchy is not detected", () => {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "codex-omarchy-noop-"));
  try {
    ...
    assert.equal(fs.existsSync(path.join(home, ".config", "omarchy")), false);
...
test("prelaunch hook installs the template, refreshes missing CSS, and preserves local edits", () => {
  ...
  try {
    ...
    fs.rmSync(generated);
    const second = spawnSync(BASH, [path.join(INTEGRATION_DIR, "install-template.sh")], {
      env,
      encoding: "utf8",
    });
