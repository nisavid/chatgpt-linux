...
This integration makes `Last updated` sort both project groups and their task rows
by recency. `Priority` and `Manual order` keep upstream's saved project-group
ordering behavior.

The integration is disabled by default because it intentionally changes upstream
sidebar semantics. Enable it in `port-integrations/integrations.json`:
