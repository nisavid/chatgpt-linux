...
test("feature is disabled until selected", () => {
  ...
  withFeatureConfig([], () => {
    assert.equal(
      loadPortIntegrationPatchDescriptors({ integrationsRoot }).some(
        (descriptor) =>
          descriptor.id ===
          "integration:project-group-last-updated-sort:last-updated-project-groups",
      ),
      false,
    );
  });
