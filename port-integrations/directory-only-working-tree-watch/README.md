...
The integration keeps recursive change detection by maintaining directory watches
...
watches, and this integration adds small directory watches around the Git index and
...
is 65536. The shared process budget also includes this integration's Git index and
...
Pure-flake selection currently uses this integration's shipped defaults; the
per-integration settings below are available through the regular
`port-integrations/integrations.json` flow, not the Nix module option.
...
- Transient operating-system resource failures during Git metadata discovery,
  ...
  Intentional exhaustion of this integration's configured watch budget uses the
...
```bash
node --test port-integrations/directory-only-working-tree-watch/test.js
```
