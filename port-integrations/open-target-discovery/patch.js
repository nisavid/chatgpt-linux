...
function guardedDesktopArgsSource({ pathVar }) {
  return `function chatgptLinuxDesktopArgs(e,t){t=chatgptLinuxOpenTargetPath(t);let n=[],r=chatgptLinuxPathToFileUri(t);for(let a of e){if(a===\`%%\`){n.push(\`%\`);continue}if(/^%[fF]$/u.test(a)){n.push(t);continue}if(/^%[uU]$/u.test(a)){n.push(r);continue}if(/^%[dD]$/u.test(a)){n.push((0,${pathVar}.dirname)(t));continue}if(/^%[nN]$/u.test(a)){n.push((0,${pathVar}.basename)(t));continue}if(/%[fFuUdDnN]/u.test(a))throw Error(\`Unsafe embedded desktop-entry field code\`);if(/^%[ickvm]$/u.test(a))continue;let o=a.replace(/%%/gu,\`%\`).replace(/%[A-Za-z]/gu,\`\`);o&&n.push(o)}return n}`;
}
...
function upgradeOpenTargetPathGuard(currentSource, deps) {
  ...
  for (const [helperName, legacySource, guardedSource] of [
    ...
    ["chatgptLinuxLaunchDesktopEntry", legacyLaunchDesktopEntrySource(), guardedLaunchDesktopEntrySource(deps)],
...
function applyIdeDiscoveryPatch(currentSource, deps) {
  ...
  const dynamicDiscoveryHelpers = patchedSource.includes("function chatgptLinuxDiscoveredIdeTargets(")
    ...
    guardedDesktopArgsSource({ pathVar }) +
    ...
    guardedLaunchDesktopEntrySource({ pathVar }) +
