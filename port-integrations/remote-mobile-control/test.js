...
const {
  ...
  applyLinuxRemoteControlIntegrationSyncPatch,
...
test("remote mobile README assigns every descriptor to one control topology", () => {
  ...
  const expected = new Map([
    ...
    ["linux-remote-control-integration-sync", "shared-boundary"],
  ...
  assert.doesNotMatch(readme, /applyLinuxRemoteControlSshInstallReleasePatch/);
...
function syntheticAppMainIntegrationSyncBundle() {
  return [
...
function syntheticSettingsRefreshBundle() {
  return [
    "var Qn=15e3,Z=React;",
    "function tr(){let $=useEffectEvent(async e=>{await P(`refresh-remote-connections`,{signal:e})}),x;",
    "cache[1]===$?x=cache[2]:(x=()=>{let e=null,t=!1,n=async()=>{t||(t=!0,e=new AbortController,await(async()=>{await $(e.signal)})().finally(()=>{e=null,t=!1}))},r=window.setInterval(()=>{n()},Qn);return()=>{e?.abort(),window.clearInterval(r)}},cache[1]=$,cache[2]=x);",
    "(0,Z.useEffect)(x,[]);return null}",
  ].join("");
}
...
function syntheticLatestSettingsRefreshBundle() {
  return [
    "var qs=15e3,As=React;",
    "function settings(){let Kt=!1,U=!0,S=send,yn=async e=>{if(!Kt)try{let t=[];t.push(S(`refresh-remote-connections`,{signal:e})),U&&t.push(S(`refresh-remote-control-connections`,{signal:e})),await Promise.all(t)}catch(e){if(e instanceof DOMException&&e.name===`AbortError`)return}},",
    "bn=(0,As.useEffectEvent)(yn),xn;e[105]===bn?xn=e[106]:(xn=()=>{let e=null,t=!1,n=async()=>{t||(t=!0,e=new AbortController,await(async()=>{await bn(e.signal)})().finally(()=>{e=null,t=!1}))},r=window.setInterval(()=>{n()},qs);return()=>{e?.abort(),window.clearInterval(r)}},e[105]=bn,e[106]=xn);",
    "(0,As.useEffect)(xn,[])}",
  ].join("");
}

function syntheticLatestSettingsUxBundle() {
  return [
    "function bs(e,t){return e.displayName.localeCompare(t.displayName)}",
    "function $a({action:e,disabled:t,hostId:n,installCodexPending:r,onAuthenticate:i,onInstallCodex:a,onReconnect:o,onRestart:s}){if(e==null)return null;switch(e.kind){case`install-codex`:return{disabled:t,label:e.label,loading:r,loadingLabel:e.loadingLabel,renderInElectronOnly:!0,tooltipText:e.tooltipText,onClick:()=>a(n)};case`login`:return{label:e.label,onClick:()=>i(n)};case`restart`:return{label:e.label,onClick:s};case`reconnect`:return{label:e.label,onClick:o};case`settings`:return null}}",
    "let Bt=H(`install-remote-codex`);let v=(C||!I)&&(ae?.code===`remote-codex-not-found`||ae?.code===`update-required`),next;",
    "let title={defaultMessage:`Control this Mac`};",
  ].join("");
}
...
function syntheticAppServerManagerSignalsBundle() {
  return [
    "const currentThreadRuntimeStatusModel=`threadRuntimeStatus:e.threadRuntimeStatus`,currentNeedsResumeModel=\"t===`needs_resume`?n?.type===`active`\";",
    "function cleanup(){}function hLn(e,t,n,r){let i={method:n,params:r};switch(i.method){case`turn/started`:{let{threadId:n,turn:r}=i.params,a=I(n);if(!t.threadStore.conversations.get(a)){e.logger.error(`Received turn/started for unknown conversation`,{safe:{conversationId:a},sensitive:{}});break}e.markConversationStreaming(a),e.updateConversationState(a,e=>{});break}case`turn/completed`:{let{threadId:n,turn:r}=i.params,a=I(n);if(!t.threadStore.conversations.get(a)){cleanup(e.getHostId(),n,r.id),t.unread.discardTurn(a,r.id),e.logger.error(`Received turn/completed for unknown conversation`,{safe:{conversationId:a},sensitive:{}});break}e.updateConversationState(a,e=>{});break}case`item/started`:{let{item:n,threadId:r}=i.params,s=I(r);if(!t.threadStore.conversations.get(s)){e.logger.error(`Received item/started for unknown conversation`,{safe:{conversationId:s},sensitive:{}});break}e.markConversationStreaming(s),e.updateConversationState(s,e=>{});break}case`item/completed`:{let{item:n,threadId:r}=i.params,s=I(r);if(n.type===`commandExecution`&&t.itemStreamState.clearItemTerminalInputBuffer(s,n.id),!t.threadStore.conversations.get(s)){e.logger.error(`Received item/completed for unknown conversation`,{safe:{conversationId:s},sensitive:{}});break}e.updateConversationState(s,e=>{});break}}}",
    "class T{unread={discardTurn(){}};itemStreamState={clearItemTerminalInputBuffer(){}};logger=z;getHostId(){return this.hostId}get notificationContext(){return{threadStore:{conversations:this.conversations},itemStreamState:this.itemStreamState,unread:this.unread,upsertConversationFromThread:this.upsertConversationFromThread.bind(this)}}onNotification(e,t){hLn(this,this.notificationContext,e,t)}}",
  ].join("");
}

function syntheticCurrentNotificationHandlerBundle() {
  return "function hLn(e,t,n,r){let i={method:n,params:r};switch(i.method){case`turn/started`:{let{threadId:n,turn:r}=i.params,a=Wl(n);if(!t.threadStore.conversations.get(a)){e.logger.error(`Received turn/started for unknown conversation`,{safe:{conversationId:a},sensitive:{}});break}break}case`item/started`:{let{item:n,threadId:r}=i.params,s=Wl(r);if(!t.threadStore.conversations.get(s)){e.logger.error(`Received item/started for unknown conversation`,{safe:{conversationId:s},sensitive:{}});break}break}case`item/completed`:{let{item:n,threadId:r}=i.params,s=Wl(r);if(n.type===`commandExecution`&&t.itemStreamState.clearItemTerminalInputBuffer(s,n.id),!t.threadStore.conversations.get(s)){e.logger.error(`Received item/completed for unknown conversation`,{safe:{conversationId:s},sensitive:{}});break}break}case`turn/completed`:{let{threadId:n,turn:r}=i.params,a=Wl(n);if(!t.threadStore.conversations.get(a)){LIn(e.getHostId(),n,r.id),t.unread.discardTurn(a,r.id),e.logger.error(`Received turn/completed for unknown conversation`,{safe:{conversationId:a},sensitive:{}});break}break}}}";
}

function syntheticCompletedItemRecoveryBundle() {
  return [
    "class U{onNotification(e,t){let n={method:e,params:t};switch(n.method){case`item/completed`:{let{item:e,threadId:t,turnId:r}=n.params,a=qf(t);",
    "this.updateConversationState(a,t=>{let n=yC(t,e=>e.turnId===r);if(!n)return;let a=nun({item:e,threadsById:this.threadStore.threadsById}),s=aC(a);",
    "LQn(e)&&(n.firstTurnWorkItemStartedAtMs=n.firstTurnWorkItemStartedAtMs??Date.now()),!(e.type!==`subAgentActivity`&&(e.type!==`sleep`||t.mode!==`durable`)&&!aD(n,e.id,e.type,$.logger))&&(e.type,AC(n,s))});break}",
    "case`item/agentMessage/delta`:break}}}",
    "const diagnostic=`Item not found in turn state`;",
  ].join("");
}
...
test("remote mobile control integration exposes descriptors when default-enabled", () => {
  withTempIntegrationRoot([], (root) => {
    assert.ok(loadPortIntegrationPatchDescriptors({ integrationsRoot: root }).length > 0);
  });
});

test("remote mobile control integration exposes its stage hook when enabled", () => {
...
test("remote mobile control integration exposes opt-in main-bundle and webview patches", () => {
  withTempIntegrationRoot(["remote-mobile-control"], (root) => {
    const descriptors = loadPortIntegrationPatchDescriptors({ integrationsRoot: root });
    assert.deepEqual(descriptors.map((descriptor) => descriptor.id), [
      "integration:remote-mobile-control:linux-remote-control-device-key",
      "integration:remote-mobile-control:linux-remote-control-client-revocation-recovery",
      "integration:remote-mobile-control:linux-remote-mobile-app-server-remote-control",
      "integration:remote-mobile-control:linux-remote-control-load-gate",
      "integration:remote-mobile-control:linux-remote-control-integration-sync",
      "integration:remote-mobile-control:linux-remote-control-visibility",
      "integration:remote-mobile-control:linux-remote-control-copy",
      "integration:remote-mobile-control:linux-remote-control-settings-ux",
      "integration:remote-mobile-control:linux-remote-control-client-revoke-setup-reset",
      "integration:remote-mobile-control:linux-remote-connections-refresh",
      "integration:remote-mobile-control:linux-remote-mobile-reasoning-summary-none",
      "integration:remote-mobile-control:linux-remote-mobile-conversation-hydration",
      "integration:remote-mobile-control:linux-remote-mobile-completed-item-recovery",
      "integration:remote-mobile-control:linux-remote-terminal-status-recovery",
      "integration:remote-mobile-control:linux-remote-control-status-read-guard",
      "integration:remote-mobile-control:linux-remote-control-status-wait",
      "integration:remote-mobile-control:linux-remote-control-enable-for-host-params",
      "integration:remote-mobile-control:linux-remote-control-enablement-bridge",
      "integration:remote-mobile-control:linux-remote-mobile-active-status",
    ]);
    ...
    const visibilityDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-visibility"
    );
    ...
    const copyDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-copy"
    );
    ...
    const featureSyncDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-integration-sync"
    );
    ...
    const enableForHostDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-enable-for-host-params"
    );
    ...
    const enablementBridgeDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-enablement-bridge"
    );
    ...
    const activeStatusDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-mobile-active-status"
    );
    ...
    const statusGuardDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-status-read-guard"
    );
    ...
    const statusWaitDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-status-wait"
    );
    ...
    const hydrationDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-mobile-conversation-hydration"
    );
    ...
    const completedItemDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-mobile-completed-item-recovery"
    );
    ...
    const terminalStatusDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-terminal-status-recovery"
    );
    ...
    const loadGateDescriptor = descriptors.find((descriptor) =>
      descriptor.id === "integration:remote-mobile-control:linux-remote-control-load-gate"
    );
...
test("Linux remote-control device-key patch fails closed when the current guard drifts", () => {
  const source = syntheticMainBundle().replace(
    "Remote control device keys are only available on macOS and Windows",
    "Remote control device keys are unavailable on this platform",
  );
  const { result, warnings } = captureWarnings(() => applyLinuxRemoteControlDeviceKeyPatch(source));
...
test("Linux remote-control feature sync forces remote_control and preserves remote_plugin on Linux", () => {
  ...
  assert.equal(applyLinuxRemoteControlIntegrationSyncPatch(patched), patched);
});
...
test("Linux remote-control settings UX patch accepts the 26.810 SSH install contract without warnings", () => {
  const source = syntheticLatestSettingsUxBundle();
  ...
  assert.match(result, /chatgptLinuxRemoteControlSshInstallActions/);
  assert.doesNotMatch(result, /chatgptLinuxRemoteControlSshInstallRelease/);
  assert.equal(applyLinuxRemoteControlSettingsUxPatch(result), result);
});

test("Linux remote-connections refresh patch handles the 26.810 compiled effect", () => {
  const source = syntheticLatestSettingsRefreshBundle();
  const { result, warnings } = captureWarnings(() => applyLinuxRemoteConnectionsRefreshPatch(source));

  assert.notEqual(result, source);
  assert.deepEqual(warnings, []);
  assert.match(result, /qs=5e3/);
  assert.match(result, /chatgptLinuxRemoteConnectionsRefreshNow/);
  assert.match(result, /document\.addEventListener\(`visibilitychange`,chatgptLinuxRemoteConnectionsRefreshNow\)/);
  assert.match(result, /window\.addEventListener\(`resume`,chatgptLinuxRemoteConnectionsRefreshNow\)/);
  assert.equal(applyLinuxRemoteConnectionsRefreshPatch(result), result);
});
...
test("Linux remote mobile conversation hydration patch handles current app-server signal shape", () => {
  ...
  assert.notEqual(patched, source);
  assert.doesNotMatch(patched, /chatgptLinuxRemoteMobileThreadRuntimeStatus/);
  assert.match(patched, /threadRuntimeStatus:e\.threadRuntimeStatus/);
  assert.match(patched, /t===`needs_resume`\?n\?\.type===`active`/);
  ...
  assert.match(patched, /e\.readThread\(a,\{includeTurns:!0\}\)/);
  ...
  assert.match(patched, /t\.upsertConversationFromThread\(chatgptLinuxRemoteMobileThread\)/);
  assert.match(patched, /e\.chatgptLinuxRemoteMobileInFlightHydrations\?\.delete\(a\)/);
  assert.match(patched, /for\(let chatgptLinuxRemoteMobileEvent of chatgptLinuxRemoteMobileQueued\)e\.onNotification/);
  ...
  assert.doesNotMatch(patched, /safe:\{[^}]*\bconversationId:/);
  assert.match(patched, /sensitive:\{conversationId:[^}]+error:/);
  assert.doesNotMatch(patched, /if\(!this\.conversations/);
...
test("Linux remote mobile conversation hydration fails closed when current storage ownership drifts", () => {
  const source = syntheticCurrentNotificationHandlerBundle().replaceAll(
    ".threadStore.conversations",
    ".threadCache.conversations",
  );
  const { result, warnings } = captureWarnings(() =>
  ...
  assert.equal(result, source);
  assert.equal(warnings.filter((warning) => warning.includes("unknown ")).length, 4);
});

test("Linux remote mobile hydration reads the explicit current thread id", async () => {
  const source = syntheticAppServerManagerSignalsBundle();
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(source);
  const context = {
    module: { exports: {} },
  ...
  manager.upsertConversationFromThread = (thread) => manager.conversations.set(thread.id, thread);
...
test("Linux remote mobile hydration uses the captured current thread id normalizer", async () => {
  const source = syntheticAppServerManagerSignalsBundle().replaceAll("I(", "J(");
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(source);

  assert.match(patched, /a=J\(n\)/);
  assert.match(patched, /s=J\(r\)/);
  assert.doesNotMatch(patched, /[as]=I\([nr]\)/);
...
test("Linux remote mobile hydration uses the captured notification variable", () => {
  const source = syntheticAppServerManagerSignalsBundle()
    .replace("let i={method:n,params:r};switch(i.method)", "let q={method:n,params:r};switch(q.method)")
    .replaceAll("=i.params", "=q.params");
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(source);

  assert.equal((patched.match(/chatgptLinuxRemoteMobileQueue\.push\(q\)/gu) ?? []).length, 4);
  assert.doesNotMatch(patched, /chatgptLinuxRemoteMobileQueue\.push\(i\)/u);
});

test("Linux remote mobile hydration bounds each pending notification queue", () => {
  ...
  const context = {
    ...
    I: (value) => value,
    setTimeout,
    z: { error() {}, warning: (message) => warnings.push(String(message)) },
  };
  vm.runInNewContext(`${patched};module.exports=T;`, context);
  const manager = new context.module.exports();
  manager.conversations = new Map();
  manager.upsertConversationFromThread = () => {};
  manager.chatgptLinuxRemoteMobilePendingNotifications = new Map([
    ["thread-a", Array.from({ length: 512 }, () => ({ method: "queued", params: {} }))],
  ]);
  manager.chatgptLinuxRemoteMobileInFlightHydrations = new Set(["thread-a"]);
  ...
  assert.equal(
    manager.chatgptLinuxRemoteMobilePendingNotifications.get("thread-a").length,
    512,
  );
  assert.ok(warnings.some((warning) => warning.includes("Dropping item/started")));
});

test("Linux remote mobile hydration retries only read failures", () => {
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(
    syntheticAppServerManagerSignalsBundle(),
  );
  assert.match(patched, /\.then\(chatgptLinuxRemoteMobileResult=>\{/u);
  assert.match(patched, /\},chatgptLinuxRemoteMobileError=>\{/u);
  assert.doesNotMatch(patched, /\.catch\(chatgptLinuxRemoteMobileError/u);
});

test("Linux remote mobile hydration ignores decoy current messages with a different logger owner", () => {
  const source = syntheticCurrentNotificationHandlerBundle().replaceAll(
    "e.logger.error(`Received",
    "e.audit.error(`Received",
  );
  const { result, warnings } = captureWarnings(() =>
    applyLinuxRemoteMobileConversationHydrationPatch(source),
  );
...
test("Linux remote mobile hydration uses normalized current thread ids", async () => {
  const source = syntheticAppServerManagerSignalsBundle();
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(source);
  const context = {
    module: { exports: {} },
  ...
  manager.onNotification("turn/started", {
    threadId: "thread-a",
    turn: { id: "turn-a", threadId: "decoy-thread" },
  });
  await new Promise((resolve) => setImmediate(resolve));
...
test("Linux remote mobile hydration restarts when a pending queue exists without an in-flight read", async () => {
  ...
  assert.equal(manager.chatgptLinuxRemoteMobilePendingNotifications?.has("thread-a"), false);
  assert.equal(manager.chatgptLinuxRemoteMobileInFlightHydrations?.has("thread-a"), false);
  assert.deepEqual(updatedConversations, ["thread-a", "thread-a"]);
});

test("Linux remote mobile hydration uses one read for concurrent current notifications", async () => {
  const source = syntheticAppServerManagerSignalsBundle();
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(source);
  const context = {
    module: { exports: {} },
...
test("Linux remote mobile hydration preserves duplicate current notifications", async () => {
  const source = syntheticAppServerManagerSignalsBundle();
  const patched = applyLinuxRemoteMobileConversationHydrationPatch(source);
  const context = {
    module: { exports: {} },
  ...
  assert.equal(manager.chatgptLinuxRemoteMobilePendingNotifications.get("thread-a").length, 2);
  assert.equal(
    manager.chatgptLinuxRemoteMobilePendingNotifications.get("thread-a")[1].params.turn.marker,
    "latest",
  );
...
test("Linux remote mobile conversation hydration patch retries transient and missing thread reads", () => {
  ...
  assert.match(patched, /if\(chatgptLinuxRemoteMobileAttempt<12\)/);
  assert.match(
    patched,
    /setTimeout\(\(\)=>chatgptLinuxRemoteMobileRead\(chatgptLinuxRemoteMobileAttempt\+1\),250\)/,
  );
...
test("Linux remote mobile conversation hydration patch warns when only part of the queue drifted", () => {
  const source = syntheticAppServerManagerSignalsBundle().replace(
    "if(!t.threadStore.conversations.get(a)){cleanup(e.getHostId(),n,r.id),t.unread.discardTurn(a,r.id),e.logger.error(`Received turn/completed for unknown conversation`,{safe:{conversationId:a},sensitive:{}});break}",
    "if(!t.threadStore.conversations.get(a)){cleanup(e.getHostId(),n,r.id),t.unread.discardTurn(a,r.id),e.logger.error(`Received turn/completed for unknown conversation`,{safe:{id:a},sensitive:{}});break}",
  );
...
test("remote mobile completed-item recovery restores a missing started item", () => {
  ...
  assert.match(patched, /chatgptLinuxCompletedItemExists=n\.items\.some\(chatgptLinuxCompletedItemCandidate=>chatgptLinuxCompletedItemCandidate\.id===s\.id\)/);
  assert.match(
    patched,
    /if\(e\.type!==`subAgentActivity`&&\(e\.type!==`sleep`\|\|t\.mode!==`durable`\)&&chatgptLinuxCompletedItemExists&&!aD\(n,e\.id,e\.type,\$\.logger\)\)return;AC\(n,s\)/,
  );
  ...
  vm.runInNewContext(
    [
      "let errors=[];",
      "var $={logger:{error:(message,details)=>errors.push({message,details})}};",
      "function qf(e){return e}",
      "function yC(e,t){return e.turns.find(t)}",
      "function nun({item:e}){return {type:e.type,id:e.id,text:e.text??null}}",
      "function aC(e){return e}",
      "function LQn(){return true}",
      "function AC(e,t){let n=e.items.findIndex(e=>e.id===t.id);n>=0?e.items[n]=t:e.items.push(t)}",
      "function aD(e,t,n,r){let i=e.items.find(e=>e.id===t&&e.type===n);if(i)return i;r.error(`Item not found in turn state`,{safe:{itemId:t},sensitive:{}});return null}",
      patched,
...
test("remote mobile completed-item recovery preserves current durable sleep semantics", () => {
  const source = syntheticCompletedItemRecoveryBundle();
  const { result, warnings } = captureWarnings(() =>
  ...
  assert.notEqual(result, source);
  assert.deepEqual(warnings, []);
  assert.match(result, /chatgptLinuxCompletedItemExists=n\.items\.some\(chatgptLinuxCompletedItemCandidate=>chatgptLinuxCompletedItemCandidate\.id===s\.id\)/);
  assert.match(result, /e\.type!==`sleep`\|\|t\.mode!==`durable`/);
  assert.equal(applyLinuxRemoteMobileCompletedItemRecoveryPatch(result), result);
});
  ...
  await withTempIntegrationRootAsync(["remote-mobile-control"], async (root) => {
    ...
    try {
      ...
      fs.writeFileSync(
        ...
        syntheticLatestSettingsUxBundle(),
      );
      fs.writeFileSync(
      ...
      const settingsPatch = report.patches.find(
        (patch) => patch.name === "integration:remote-mobile-control:linux-remote-control-settings-ux",
      );
      assert.equal(settingsPatch.sourceKind, "integration");
      assert.equal(settingsPatch.integrationId, "remote-mobile-control");
      assert.equal(settingsPatch.status, "applied");
      assert.deepEqual(settingsPatch.warnings ?? [], []);
      ...
      const integrationHydrationPatch = report.patches.find(
        (patch) => patch.name === "integration:remote-mobile-control:linux-remote-mobile-conversation-hydration",
      );
      assert.equal(integrationHydrationPatch.sourceKind, "integration");
      assert.equal(integrationHydrationPatch.status, "applied");

      const completedItemPatch = report.patches.find(
        (patch) => patch.name === "integration:remote-mobile-control:linux-remote-mobile-completed-item-recovery",
      );
      assert.equal(completedItemPatch.sourceKind, "integration");
      assert.equal(completedItemPatch.integrationId, "remote-mobile-control");
  ...
  await withTempIntegrationRootAsync(["remote-mobile-control"], async (root) => {
    await withIntegrationRootEnvAsync(root, async () => {
      ...
      try {
        ...
        assert.match(patchedSignalsFile, /threadRuntimeStatus:e\.threadRuntimeStatus/);
        assert.match(patchedSignalsFile, /t===`needs_resume`\?n\?\.type===`active`/);
        ...
        assert.equal(
          report.patches.some(
            (patch) => patch.name === "integration:remote-mobile-control:linux-remote-control-preserve-config",
          ),
          false,
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-mobile-app-server-remote-control" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-load-gate" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-integration-sync" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-visibility" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-copy" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-settings-ux" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-client-revoke-setup-reset" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-connections-refresh" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
        ...
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-mobile-conversation-hydration" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
        ...
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-terminal-status-recovery" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-status-read-guard" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-status-wait" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-control-enablement-bridge" &&
            patch.status === "applied",
          ),
        );
        assert.ok(
          report.patches.some((patch) =>
            patch.name === "integration:remote-mobile-control:linux-remote-mobile-active-status" &&
            patch.status === "applied",
          ),
        );
