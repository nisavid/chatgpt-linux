...
const INTEGRATION_PICKER_ON_UPDATE_SETTING_KEY = linuxSettingsKeys.integrationPickerOnUpdate;
...
function applyMainBundlePatch(source) {
  ...
  const helper = [
    ...
    `function chatgptLinuxWrapAppStateDir(){let __chatgptWrapExplicitStateDir=process.env.CHATGPT_LINUX_APP_STATE_DIR;if(typeof __chatgptWrapExplicitStateDir===\`string\`&&__chatgptWrapExplicitStateDir.trim())return __chatgptWrapExplicitStateDir;let __chatgptWrapHome=chatgptLinuxWrapHome();let __chatgptWrapStateRoot=process.env.XDG_STATE_HOME||(__chatgptWrapHome&&chatgptLinuxWrapPath().join(__chatgptWrapHome,\`.local\`,\`state\`));return __chatgptWrapStateRoot?chatgptLinuxWrapPath().join(__chatgptWrapStateRoot,chatgptLinuxWrapAppId()):null}`,
    `function chatgptLinuxWrapStatePath(){let __chatgptWrapHome=chatgptLinuxWrapHome();let __chatgptWrapStateRoot=process.env.XDG_STATE_HOME||(__chatgptWrapHome&&chatgptLinuxWrapPath().join(__chatgptWrapHome,\`.local\`,\`state\`));return __chatgptWrapStateRoot?chatgptLinuxWrapPath().join(__chatgptWrapStateRoot,\`chatgpt-updater\`,\`state.json\`):null}`,
    `function chatgptLinuxWrapMarkerPath(){let __chatgptWrapStateDir=chatgptLinuxWrapAppStateDir();return __chatgptWrapStateDir?chatgptLinuxWrapPath().join(__chatgptWrapStateDir,\`chatgpt-wrapper-updater\`,\`pending\`):null}`,
    `function chatgptLinuxWrapReadStatus(){try{let __chatgptWrapStatePath=chatgptLinuxWrapStatePath();if(!__chatgptWrapStatePath||!chatgptLinuxWrapFs().existsSync(__chatgptWrapStatePath))return null;return JSON.parse(chatgptLinuxWrapFs().readFileSync(__chatgptWrapStatePath,\`utf8\`))}catch{return null}}`,
    ...
    `function chatgptLinuxWrapSpawnCheck(){if(!chatgptLinuxWrapWrapperUpdatesEnabled()||!chatgptLinuxWrapManagerAvailable())return;try{let __chatgptWrapCheckProcess=chatgptLinuxWrapChildProcess().spawn(chatgptLinuxWrapManagerPath(),[\`check-wrapper\`],{stdio:\`ignore\`,detached:!0,env:process.env});__chatgptWrapCheckProcess.on(\`error\`,()=>{});__chatgptWrapCheckProcess.unref()}catch{}}`,
    ...
    `function chatgptLinuxWrapSettingsPath(){let __chatgptWrapSettingsFile=process.env.CHATGPT_LINUX_SETTINGS_FILE;if(typeof __chatgptWrapSettingsFile===\`string\`&&__chatgptWrapSettingsFile.length>0)return __chatgptWrapSettingsFile;let __chatgptWrapHome=chatgptLinuxWrapHome();let __chatgptWrapConfigRoot=process.env.XDG_CONFIG_HOME||(__chatgptWrapHome&&chatgptLinuxWrapPath().join(__chatgptWrapHome,\`.config\`));return __chatgptWrapConfigRoot?chatgptLinuxWrapPath().join(__chatgptWrapConfigRoot,chatgptLinuxWrapSettingsAppId(),\`settings.json\`):null}`,
    ...
    `function chatgptLinuxWrapPickerEnabled(){try{let __chatgptWrapSettingsPath=chatgptLinuxWrapSettingsPath();if(!__chatgptWrapSettingsPath||!chatgptLinuxWrapFs().existsSync(__chatgptWrapSettingsPath))return!0;let s=JSON.parse(chatgptLinuxWrapFs().readFileSync(__chatgptWrapSettingsPath,\`utf8\`));if(!s||typeof s!==\`object\`)return!0;let v=s[\`chatgpt-linux-integration-picker-on-update\`];if(v==null)return!0;if(typeof v===\`boolean\`)return v;if(typeof v===\`number\`)return v!==0;if(typeof v===\`string\`){let n=v.trim().toLowerCase();return!([\`0\`,\`false\`,\`no\`,\`off\`].includes(n))}return!0}catch{return!0}}`,
    ...
    `function chatgptLinuxWrapWriteMarker(){let __chatgptWrapMarkerPath=chatgptLinuxWrapMarkerPath();if(!__chatgptWrapMarkerPath)return{ok:!1,reason:\`no-marker-path\`};try{chatgptLinuxWrapFs().mkdirSync(chatgptLinuxWrapPath().dirname(__chatgptWrapMarkerPath),{recursive:!0});chatgptLinuxWrapFs().writeFileSync(__chatgptWrapMarkerPath,new Date().toISOString());return{ok:!0,path:__chatgptWrapMarkerPath}}catch(e){return{ok:!1,error:String(e?.message||e)}}}`,
    `function chatgptLinuxWrapInstallNow(){if(!chatgptLinuxWrapWrapperUpdatesEnabled())return{ok:!1,reason:\`wrapper-updates-disabled\`};if(!chatgptLinuxWrapManagerAvailable())return{ok:!1,reason:\`updater-unavailable\`};if(chatgptLinuxWrapHasDisplay()&&chatgptLinuxWrapPickerEnabled())chatgptLinuxWrapRunPicker();let __chatgptWrapMarker=chatgptLinuxWrapWriteMarker();if(!__chatgptWrapMarker.ok)return __chatgptWrapMarker;try{let __chatgptWrapElectronApp=require(\`electron\`).app;setTimeout(()=>__chatgptWrapElectronApp.exit(0),120);return{ok:!0,path:__chatgptWrapMarker.path}}catch(e){return{ok:!1,error:String(e?.message||e)}}}`,
...
function wrapperRuntimeSource() {
  return [
    ...
    `function dispatch(payload){let bridge=window.electronBridge,ev=new CustomEvent("codex-message-from-view",{detail:payload});if(bridge?.sendMessageFromView){ev.__chatgptForwardedViaBridge=!0;bridge.sendMessageFromView(payload).catch(()=>{})}window.dispatchEvent(ev)}`,
...
function applyWrapperUpdateSettingsPatch(source) {
  ...
  if (!next.includes("integrationPickerOnUpdate:")) {
    ...
    next = next.replace(
      wrapperKey,
      `${wrapperKey},integrationPickerOnUpdate:${JSON.stringify(INTEGRATION_PICKER_ON_UPDATE_SETTING_KEY)}`,
    );
  }
...
module.exports = {
  HANDLER_NAME,
  RUNTIME_VERSION,
  INTEGRATION_PICKER_ON_UPDATE_SETTING_KEY,
