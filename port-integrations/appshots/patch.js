...
function applyLinuxAppshotAvailabilityPatch(currentSource) {
  ...
  if (
    (currentSource.includes("codexLinuxAppshotsPlatformAvailable") && !hasLegacyGate) ||
  ...
  if (changed) {
    return needsPlatformHelper
      ? `${patchedSource}function codexLinuxAppshotsPlatformAvailable(e){return e===\`macOS\`||e===\`linux\`}`
      : patchedSource;
  }
...
let u=null;
try{
u=n.mkdtempSync(i.join(r.tmpdir(),\`codex-appshot-\`));try{n.chmodSync(u,448)}catch{}let d=i.join(u,\`source.png\`),f=i.join(u,\`crop.png\`),p=c.output===\`append\`?[...c.args,d]:[...c.args,...c.output,d];
