...
Default-on port integration for voice-in/voice-out conversation mode in normal
Codex turns.

This integration does not add a new model provider or a new agent tool. It reuses the
...
Add the integration id to `port-integrations/integrations.json` before installing or
rebuilding:
...
- Leaves typing available; typed turns still work normally while the integration is
  enabled.

This is intentionally separate from OpenAI-hosted realtime voice services. It
keeps the integration thin and local while still giving the user one
...
```bash
node --test port-integrations/conversation-mode/test.js
```
