...
help:
	...
	@printf '  %-18s %s\n' "make build-app" "Run install.sh and regenerate codex-app/ (reuses cached ChatGPT.dmg)"
...
clean:
	...
	rm -rf "$(CURDIR)/codex-app" "$(CURDIR)/ChatGPT.dmg" "$(CURDIR)/dist" "$(NEXT_APP_DIR)" "$(REBUILD_REPORT_DIR)"
