...
CODEX_APP_ID="${CODEX_APP_ID:-codex-app}"
CODEX_APP_DISPLAY_NAME="${CODEX_APP_DISPLAY_NAME:-Codex App}"
INSTALL_ROOT="${CODEX_INSTALL_ROOT:-$SCRIPT_DIR}"
DEFAULT_INSTALL_DIR_NAME="codex-app"
DEFAULT_CODEX_WEBVIEW_PORT=5175
if [ "$CODEX_APP_ID" != "codex-app" ]; then
...
main() {
    ...
    if [ "$INSPECT_ONLY" -eq 1 ]; then
        check_inspect_deps
    elif [ "${CODEX_INSTALL_TRANSACTION_ACTIVE:-0}" != "1" ]; then
        check_deps
